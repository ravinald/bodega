...
s3://<bucket>/
  ...
  gomod/                     # Go module archives, module path verbatim:
                             #   gomod/example.com/example-corp/sdk/@v/v1.30.0.{zip,info,mod}
...
The staging file and the directory it is written in are reduced to their mode bits before the body is written, in whichever ACL type the filesystem keeps, and read back to confirm it. A `chmod` does not do that everywhere: ZFS at `aclmode=passthrough` keeps every entry the storage root handed down, so a named grant on the root reached the replacement body for the length of the write. An NFSv4 ACL is set to its trivial form rather than deleted, because `__acl_delete_fd(ACL_TYPE_NFS4)` on ZFS returns success and leaves the entries in place. A staging inode that still carries a named or inheritable entry fails the write, and so does one on a FreeBSD filesystem that reports neither ACL type: with nothing to read back, a filesystem that keeps no ACL cannot be told from one whose `fpathconf` answer is wrong, and passing the second is the silent no-op this check replaced. A fresh object is not staged this way: it is the file its directory would have created, and carries after the rename what it carried before.
...
Two alternatives were rejected. Leaving the codename in `apt_suites` while `apt_upstreams` takes over its `dists/` tree, with bodega simply not signing it, gives silent partial service: the locally built `.deb`s stay in the store and `bodega status apt` keeps listing them while the forwarded `Packages` names only upstream packages, so nothing serves them and every diagnostic reports success. Merging the two indexes under bodega's own signature is the arrangement that would put a local and an upstream `.deb` in one suite, and it costs the multi-paragraph `deb822` parser deferred in `internal/deb822`, run over an index tens of megabytes uncompressed per component and architecture; a precedence rule for a package name present on both sides, which is a supply-chain decision, because local-wins lets anyone with manifest write access shadow `libc6`; republished or stripped `by-hash/` bodies, since the upstream `Release` advertises digests of the upstream bytes; and a second refresh loop, because a merged index has to track the upstream's freshness as well as bodega's. An operator who wants both in one apt transaction gets it today by listing two suites in one `.sources` stanza.
...
| A retired `tls_autocert: true` or `tls_domain`, or `--tls-autocert`/`--tls-domain`, with no cert pair | The option that promised TLS promises nothing; the server binds in the clear or refuses                      | `reportRetiredTLSKeys`, `cmd/bodega/cmd_serve.go` |
...
[Threat model](threat-model.md). Operators standing up locked-down
...
| `cidr`  | a masked CIDR      | "everything on this subnet is a devbox" | no bootstrap problem; behind a proxy, needs `trusted_proxies` answered |
...
**What each client sends.** Eight routes, three header formats, and `credentialFrom` in `internal/server/identity.go` parses all three without asking which route the request landed on:
...
**A CIDR binding names a connection, not a header.** The address `resolve` matches on is the one `RealIPMiddleware` settled, so a client behind a proxy resolves to the client rather than to the proxy, and two clients behind one proxy resolve to two identities. That same middleware is where the hazard enters: `trusted_proxies` defaults to loopback plus RFC 1918, and bodega returns `X-Real-IP` verbatim from any peer in that set (see **IP resolution**), so a peer in that range could claim an address inside a bound network and collect that identity.

So the gate is per request and on the address's provenance, which is what `RealIPMiddleware` now carries in the context beside the address itself. An address read off the connection always names a host: it is whatever completed a handshake, and nothing a client writes changes it. An address read out of `X-Real-IP` or `X-Forwarded-For` names a host only once `trusted_proxies` has an answer, which is the operator naming the peers whose headers they believe. Token bindings are untouched by either branch, because a credential is a claim the caller had to hold.

Gating on config state instead was the rejected approach, and it shipped first: `resolve` asked whether `trusted_proxies` had been answered at all, and `guardCIDRBindings` refused to start an instance carrying a CIDR binding without one. It is the wrong question, because the hazard is the header rather than the address, and it made the CIDR half inert on every install that never put a proxy in front of bodega — which is the read-only host the binding kind exists for. The operator saw `bodega identity list` carrying the binding, `bodega profile show` naming the bound host, and every audit row for that host naming nobody, so the profile selected nothing and each of the eight gates passed. `acl proxies remove` is still not a way out of the built-in default, because it refuses a CIDR the list does not hold rather than claiming the list as a side effect.
...
**The same rule governs bodies, keyed rather than routed.** `RequestLogger` captures request and response bodies at `log_level: 4`, and `handleCreateToken` returns the plaintext token in its body because issuance is the one moment it can be read, so trace level wrote a write credential into the journal. Bodies are now walked as JSON and the values of `token`, `access_token`, `refresh_token`, `secret`, `password`, `private_key`, `signing_key` and `session` are replaced at any depth. Routing the rule instead — refusing body capture on the token endpoint — was rejected: it closes today's leak and leaves the next route returning a signing key to be remembered, which is the remembering that failed here. A JSON body that does not parse is scrubbed textually against the same key set rather than withheld. Withholding it was the first approach and it was wrong: `maxBodyCapture` truncates at 64KB and a handler can answer with a JSON content type over a body that is not JSON, so withholding leaves an operator debugging a malformed request with nothing to read, which is what trace level exists to prevent. The key survives a truncation ahead of the value it names, so the scrub still reaches the credential.
...
Collapsing the third level into the first makes an open set with one pinned package look impossible, and that is the ordinary case: everything tracks except postgres, held at 14 because 15 breaks the config. The same relation runs the other way, one package tracking inside a type that otherwise pins, and an entry carrying `constraint_kind: any` expresses it.

`constraint_kind` holds the four values already on `manifest.VersionEntry`: `exact`, `compatible`, `patch`, `any`. A fifth spelling of the same idea is a defect waiting for the day the two disagree about what `^` matches. Empty is the third level declining to override, which defers to the type's default. An entry with an empty `constraint_kind` still carries a `version`, and a `pinned` default reads it as the version that package is held at; a `floating` default ignores it. That is the state `bodega profile unpin` leaves behind, which is why it clears the constraint and keeps the version: erasing both under a pinned default would turn a released hold into a total refusal.
...
**A pin is a tracked decision, not an unlabeled version string.** `bodega profile pin` requires `--reason` and takes `--review-after <YYYY-MM-DD>`, both stored on the entry, and `pinned_at` (`016_profile_pinned_at.up.sql`) records when the held version was last decided. That column is separate from `created_at` because the two answer different questions: `created_at` is when the package joined the profile, and it does not move when the version does, so a pin moved yesterday under an entry written two years ago would read as two years stale to the report that measures a review date. An unparsable review date is refused at every write, which is all three: `pin`, `add`, and the entries of a `--from-file` document. The file path was the one F15 missed, and a pin stored with a date nothing parses is a pin that is never overdue, so the gate reading it passes forever.

`pinned_by` (`018_profile_pinned_by.up.sql`) is the same argument on the other axis. `actor` is assigned on every write, so a second operator correcting a typo in a reason took the byline while `pinned_at` kept the original decision date, and the row then paired one person's name with another person's decision. Both columns move together and only when the pin does. `actor` stays the last writer, which is what every profile command other than the pin report means by it; preserving it across a merge would make that column read "first writer" everywhere, wrong for the commands that are not about a pin.
...
**Concurrency.** `database/sql` pools connections, so several writers through one handle are several SQLite connections contending for the write lock. The DSN carries `busy_timeout=5000`, which makes the loser wait rather than take `SQLITE_BUSY` and lose its row. Serializing a shared handle with `SetMaxOpenConns(1)` would also work and is not used: it takes the concurrent reads WAL exists to allow, so a dashboard query would queue behind every write. On an M1 Ultra with an internal NVMe SSD, eight concurrent writers sustain ~2,600 inserts/sec through one handle.

**Two handles, because one can only have half of that.** Waiting for the lock keeps no row from being lost only while the wait outlasts the load, and that made the guarantee a property of the machine. Measured on Linux under `-race` on two cores, with the timeout as the only variable and 400 events across eight writers: 400 stored at 5s, 399 at 200ms, ~390 at 50ms, ~338 at 10ms. CI crossed that line occasionally and the conformance suite reported it as the regression the guarantee exists to prevent.

So `audit.DB` opens the file twice. The read pool stays unbounded, which is the half the paragraph above is right about. The write handle is capped at one connection, so bodega's own writes queue in Go's pool instead of racing for SQLite's lock, and the write path cannot produce `SQLITE_BUSY` against itself at any timeout. Every `ExecContext` and `BeginTx` in `internal/audit` goes through an accessor — `DB.writer`, or `sqliteSink.writer` for the four the sink owns — and so does schema setup: `PRAGMA journal_mode=WAL`, the migrations, the policy-seed claim and the checksum backfill. A migration racing the read pool is the one ordering that has to be impossible.

The split holds only while exactly one of the two pools writes, and the way that stops being true is a write added to a read path copying whichever handle its neighbour named. So the read pool is opened `query_only` on every open, not just the read-only ones: a write that reaches it is a database error at the first call rather than a second unbounded set of connections back on the write lock, costing no error and no row and showing up later as the flake this section exists to remove. `TestReadPoolRefusesWrites` asserts the refusal against a control write through `DB.writer`.

`busy_timeout` stays at 5s, because it still governs a writer this process cannot serialize away: `sqlite3` opened on the audit file, a second bodega on the same volume, a backup holding a read transaction while the WAL checkpoints. What changed is that the shipped value is no longer what stands between the fleet's own writes and a dropped row.

The conformance case that asserts this now sets its own budget, 1ms rather than 5s. At 5s it asserted two things: that the sink keeps every event, and that the machine finished inside the wait. The second is not a property of bodega and is the one that failed.
...
`binary_upstreams` is the same structure applied to `/binaries/`, and shares the validator, the modes and the defaults with `git_upstreams`. Binaries are the type most likely to come from many vendors at once — a releases host, a forge serving release assets, a vendor CDN — which is why a single flat key cannot name what an install pulls from:

```json
"binary_upstreams": {
  "example-vendor": { "url": "https://downloads.example.com/", "mode": "open" },
  "github":    { "url": "https://github.com/",             "mode": "catalog" }
}
```
