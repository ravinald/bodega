...
```bash
bodega pkg create binary example-tool-v2       # prompts for URL, SHA256
...
```bash
pip install --index-url https://bodega-host:8080/pypi/simple/ examplesdk
```

**Go modules**:

```bash
GOPROXY=https://bodega-host:8080/go go get example.com/example-corp/widget-sdk@v1.30.0
```
...
```bash
curl https://bodega-host:8080/git/widget/widget-v4.5.7.bundle -o widget.bundle
git clone widget.bundle widget
```
...
- Read [Design](design.md) for architecture details
