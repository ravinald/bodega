...
A binary entry with no `filename` is stored under its url's last segment. For a url with no path, that segment is the authority, so the object key is `binaries/<name>/<version>/user:secret@host`. The read routes publish such an entry with a `filename` of `<host>~<tag>`, and the web UI links to it. The tag is 16 hex digits of an HMAC over the package name, the version and the stored name, keyed by the server's token pepper, so it depends on that entry alone and nobody without the pepper can derive it or test a guessed credential against it. `/binaries/` resolves a name ending in `~` and 16 hex digits only as one of these aliases, never as a stored name. A saved link therefore keeps reaching the same bytes, or answers 404 once its entry is gone, whatever entries are later added, removed or reordered, including one given that name as an explicit `filename`. An entry whose own stored name already has that form gets an alias the same way, and its stored name stops answering directly. Rotating the pepper changes every tag and turns every saved link into a 404; so does a server with no pepper, which publishes a tag of zeros that resolves nowhere. The key itself still carries the credential, so anyone who can list the storage backend reads it. Set `filename` on such an entry and fetch it again to store the object under a name that carries nothing.

**Upgrading.** Nothing refuses to start and no entry is refused: a manifest that already carries a credential keeps serving and fetching as before, and only what the read API and the status refusal publish changes. A binary download at its old path, userinfo in the filename, still answers; the web UI's link now uses the published `filename` described above instead. Three consequences follow:

- A manifest fetched from the read API and pushed back through `bodega pkg import` or `POST /api/v1/packages/{type}` arrives without its credential, and the next fetch from that upstream goes out anonymous. A binary entry also arrives carrying the published `filename`, so its next fetch stores a new object under that name, and since the name has the alias form it is reachable only through the new alias the read routes publish for it. Import from the manifest file under `manifest_dir` instead.
