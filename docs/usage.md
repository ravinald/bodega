...
| freebsd | `/freebsd/{abi}/{repo}/...`                                            | every request bodega answers from the cache or proxies to upstream, one row per upstream host, which is also the pattern `promote` writes: the allow-list keys this type on the host, as it does apt. A repository no entry names is a 404 with no row, because the entry is what supplies the upstream URL and there is none to record                                                                                                                                                                                                                                                                                                                                                                               |
