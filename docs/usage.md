...
- `profile` and `host` answer for the host that asked, when a profile scopes apt for it. `host` is the one stanza that host should install, so a client installs it rather than picking a block out of `sources`. Both are absent when the answer is the operator's: no profile scopes apt and this instance serves more than one codename, and a guess there points a host at the wrong suite with nothing reporting it.
