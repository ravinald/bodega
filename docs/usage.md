...
A binary entry with no `filename` is stored under its url's last segment. For a url with no path, `https://user:secret@host`, that segment is the authority, so the object key is `binaries/<name>/<version>/user:secret@host`. The web UI builds its download link from the published url and asks for `binaries/<name>/<version>/host`; `/binaries/` maps that name to the stored key when the entry has no `filename` and no other entry of that version is stored under it. The key itself still carries the credential, so anyone who can list the storage backend reads it. Set `filename` on such an entry and fetch it again to store the object under a name that carries nothing.

**Upgrading.** Nothing refuses to start and no entry is refused: a manifest that already carries a credential keeps serving and fetching as before, and only what the read API and the status refusal publish changes. A binary download at its old path, userinfo in the filename, still answers, and the web UI's link now uses the public name described above. Three consequences follow:
