...
Downloads raw sources without building or packaging. If no types are given, all ten are fetched in dependency order: `binary, git, apt, pypi, gomod, helm, npm, cargo, freebsd, distfiles`.

When a name is given after the type, only that entry is fetched.

`fbsd` is accepted for `freebsd` by every command that takes a list of types: `build fetch`, `build run`, `build package`, `build upload`, `build sync`, `build status` and `pkg drift`. It is not a type. The help output does not list it, `manifest.AllTypes` does not hold it, and it resolves to `freebsd` before anything reads a manifest or composes a storage key, so nothing is ever stored under it. The commands that take exactly one type (`pkg create`, `show pkg`, `pkg delete` and the rest) refuse it: a spelling that reached a manifest path there would give one type two key prefixes, and whatever landed under the second is unreachable from every reader that asks for the first.
...
`bodega status` is a different command: the repository dashboard. See [`bodega status [TYPE]`](#bodega-status-type).
...
It then reads every binary manifest in the store, whether or not the index lists it, and checks each version's `filename` against the rule under [Binary-specific fields](#binary-specific-fields) and prints an `INVALID` row, with the reason, for each one that breaks it, or an `UNCHECKED` row for a manifest it cannot parse. Either row makes the command exit non-zero. Fix one by correcting `filename` or removing it with `bodega pkg edit`.
...
### `bodega pkg freeze TYPE NAME`

Toggle the `frozen` flag on every version of a package. Frozen versions cannot be built, edited, or deleted. When every version is already frozen, `freeze` unfreezes them all; otherwise it freezes them all. It takes exactly two arguments: freezing one version is done through the REST API (`PATCH /api/v1/packages/{type}/{name}/freeze/{version}`) or by setting `frozen` on that version with `bodega pkg edit` or the TUI's `E` editor.
...
### `bodega pkg edit <type> <name> [version]`

Loads the JSON for a package, or for one version when VERSION is given, into a temporary file, opens it in an editor, and re-imports it on save. Without VERSION the whole package manifest is edited, top-level fields included; with it, only the matching version entry, matched on `version` or `ref`.

| Flag       | Default | Purpose                                                                                |
| ---------- | ------- | -------------------------------------------------------------------------------------- |
| `--editor` |         | Editor to run. Resolution order: `--editor`, then `$VISUAL`, then `$EDITOR`, then `vi` |

```bash
bodega pkg edit npm @example-corp/widget-cli
bodega pkg edit npm @example-corp/widget-cli 1.5.0
bodega pkg edit --editor=nvim git widget
```

A save that changes no bytes exits without touching storage. A save that fails validation or policy leaves the edited file on disk and names it, so the change is not lost: inspect it with `--editor cat`, or copy it and retry. Changing `storage` is checked against the backends, as described under [Named backends](#named-backends-and-per-type-placement): an edit that points a version at a backend the object is not on, while the backend it names today holds it, is refused and names `bodega pkg move`.
...
### `bodega pkg export [type] [name] [version]`
...
bodega pkg export npm @example-corp/widget-cli 1.4.2   # one version
bodega pkg export apt python3 > python3.json           # save to file
```

A single package is output as a JSON object. Multiple packages are output as a JSON array. With a version, the output is the package manifest with every top-level field kept and the `versions` array holding only the match, so it round-trips through `bodega pkg import`.
...
Movable types: `binary`, `npm`, `cargo`, `gomod`, `helm`, `apt`, `git`, `freebsd`, `distfiles`. See [pypi is not movable](#pypi-is-not-movable) for the one that is not, and [a freebsd move is a republication](#a-freebsd-move-is-a-republication) for the one that does not follow the order above.
...
| `--public-url`      |         | Base URL clients reach the server at; overrides `public_url` and `BODEGA_PUBLIC_URL` |
| `--quiet`           | `false` | Suppress the startup banner on stderr. `log_level` output is unaffected              |
...
- `UNLINKED`: a git version whose source tree is on disk but which has no dependency edges, usually because it was fetched before dependency discovery ran.
- `MISSING` and `VERSION`: a graph edge naming a package, or a version of one, that no manifest holds.
- `EMPTY`: a package with no versions. `ERROR` marks one that could not be loaded.
- `ORPHAN`: a package no other package requires. This is informational and not counted as an issue, since most top-level packages are orphans by design.

```bash
bodega audit check
```

It ends with `N issue(s) found. Run 'bodega repair' to fix.` or `All dependencies satisfied.`, and exits 0 either way, so a script has to read the last line rather than the exit code. It reads the manifests and changes nothing.

### `bodega status [TYPE]`

Prints the repository dashboard: package and version counts, how many versions the storage backends hold, storage used, frozen and hidden counts, dependency edges and orphans, a per-type and a per-backend table, and the last 24 hours of fetch, build and create activity from the audit database. With a type, it prints that type's figures alone.

```bash
bodega status          # the whole repository
bodega status apt      # apt alone
```

Every configured backend is probed, `local` and `s3` alike. Under an `audit_sink` that keeps no table (`syslog`, `jsonl`) the activity line cannot be counted, and a note on stderr says so rather than printing zeros. `bodega build status` is the per-entry table this summarizes.
...
`binary`, `git`, `helm`, `freebsd` and `distfiles` have no OSV identifier. `set` refuses them:
...
`apt`, `binary`, `git`, `helm`, `freebsd` and `distfiles` have no such source. `set` refuses them:
...
| `--region`       | `AWS_REGION`           | `us-west-2`                | AWS region of the default backend                                                                            |
...
| `--local-config` |                        | `false`                    | Read and write manifests on the local filesystem instead of the configured backend, whichever driver it uses |
...
**On `local`, a restriction is a decision and a refill is not.** Refilling an object restates everything it carried that decides who can reach it: its mode, its owner and group, its access ACL and its extended attributes, applied to the staging file inside a directory nothing else can enter. The ACL goes on after the mode, because on ZFS a `chmod` rewrites an NFSv4 ACL and drops every entry the mode cannot express. So a reader the artifact denied stays denied. On FreeBSD both kinds of ACL are carried: POSIX.1e on UFS mounted `-o acls`, and NFSv4 on ZFS or UFS mounted `-o nfsv4acls`. Where the server cannot restate one of them, the refill fails, names which one, and leaves the previous object exactly as it was: handing an artifact to a group it was kept from is not something a cache fill gets to decide. Giving an artifact a group the server does not belong to is the case that reaches this, and it needs `CAP_CHOWN` or a server running as a member of that group. An ACL the filesystem under the key does not keep is the other: an NFSv4 ACL on an object sitting on a UFS that keeps POSIX.1e, or none, refuses the refill with `the object's NFSv4 ACL cannot travel to a filesystem that keeps POSIX.1e ACLs` rather than landing the artifact without it. A new object has nothing to carry and lands at the mode the server's umask allows. On FreeBSD a UFS mounted without `acls` or `nfsv4acls` keeps no ACL for the staging file to be confirmed free of, so a refill there is refused with `the filesystem reports that it keeps neither NFSv4 nor POSIX.1e ACLs` and the previous object stays; a new object still lands. Keep the storage root on ZFS, or mount the UFS with one of those options.
...
| `freebsd_root`   | freebsd   | `<root>/freebsd`                                                                 |
| `distfiles_root` | distfiles | `<root>/distfiles`, with the DISTDIR at `<root>/distfiles/@<environment digest>` |
...
**Gap:** the TUI's config form shows four of the ten roots (`apt_root`, `git_root`, `pypi_root`, `binary_root`), so `gomod_root`, `helm_root`, `npm_root`, `cargo_root`, `freebsd_root` and `distfiles_root` can only be set by editing the file. Ctrl+R clears the same four. Tracked as #227.
...
| `AWS_REGION`                                                             | AWS region of the default backend, over `region` in the config (default: `us-west-2`). A named entry in `storage_backends` with no `region` of its own takes neither the config's `region` nor that default: the AWS SDK's chain resolves one, reading this variable itself |
| `AWS_PROFILE`, `AWS_ACCESS_KEY_ID` and the rest of the AWS default chain | Credentials for an S3 backend. bodega has no credential key of its own; see [S3 setup](#s3-setup)                                                                                                                                                                           |
| `AWS_DEFAULT_REGION`                                                     | Read by the AWS SDK's chain, after `AWS_REGION`, for a named backend that sets no `region`. bodega's default backend does not read it                                                                                                                                       |
| `AWS_ENDPOINT_URL_S3`                                                    | Read by the AWS SDK: points S3 calls at an S3-compatible store instead of AWS                                                                                                                                                                                               |
| `BOOTSTRAP_BUILD_ROOT`                                                   | Local build directory (default `/opt/bodega`). Overridden by `--build-root`                                                                                                                                                                                                 |
...
| `BODEGA_PUBLIC_URL`                                                      | Base URL clients reach the server at. Overrides `public_url`; overridden by `bodega serve --public-url`                                                                                                                                                                     |
| `BODEGA_SERVER`                                                          | The bodega server a host that is not the server pushes to (`pkg import`, `pin`). Overrides `server_url`; overridden by `--server`                                                                                                                                           |
| `BODEGA_TOKEN`                                                           | Bearer token for that server. Read before `token` in the config file, so a token never has to be written to disk on a host being cataloged                                                                                                                                  |
| `BODEGA_SERVICE_USER`                                                    | The account the server runs as, for placing the token pepper where that account can read it. Overrides the `User=` read from the systemd unit, and is the only way to declare the account on a host with no unit                                                            |
...
|                                                 | Linux                                                | FreeBSD                                                                                                                         |
| ----------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Run the server                                  | yes                                                  | yes                                                                                                                             |
| Storage permissions and extended attributes     | POSIX ACLs, carried in extended attributes           | NFSv4 ACLs on ZFS, POSIX.1e ACLs on UFS mounted with `acls`, and `extattr`                                                      |
...
- **filename**: overrides the basename derived from the URL. It must be a clean relative path: subdirectories such as `~/tool` are allowed, but not a `..` or `.` segment, an empty segment (`a//b`, a trailing `/`), a leading `/`, a `\` or a NUL. The fetch joins it into the build root and the upload joins it into the object key, so a parent reference or an absolute path would move a write. Every manifest write refuses one that breaks the rule (`pkg create`, `pkg edit`, `pkg import`, the mutation API, and a write the builder makes after a fetch), and so does the fetch.
...
The build pipeline runs four stages, each a `bodega build` subcommand, processed in dependency order:

```text
fetch > run > package > upload
```

`bodega build sync` pushes whatever local artifacts exist to the storage backend without running any stage, and `bodega build status` compares the manifests against the backends that hold their artifacts.
...
**Build order:** `binary, git, apt, pypi, gomod, helm, npm, cargo, freebsd, distfiles`. This order reflects dependencies (e.g., pypi may reference git-cloned repos for its base requirements). It is `manifest.AllTypes`, and the three build subcommands render their help from it rather than restating it.
...
`apt_signing_name` and `apt_signing_email` in `config.json` supply the UID; `--name` and `--email` override them. Every `bodega apt key` subcommand takes `--path` to write or read one named file instead of searching the paths above, which is how a key is generated on a workstation for delivery through `LoadCredential=` rather than on the server. A new key takes effect on `systemctl reload bodega` (`SIGHUP`), which re-reads the key file, re-renders the served keyring and re-signs the index in one step. The rotation runbook below depends on that.
...
A record that resolves onto a repository-root file is refused as well: `meta.conf`, `data.pkg` and `packagesite.pkg`, the legacy names `digests.pkg`, `digests.txz`, `packagesite.txz` and `repo.txz` that the route answers 404 by, and the fallback names below that it answers 404 by on a hosted entry. The repository root is bodega's own namespace, and a package keyed there would land on the metadata a client reads to find every other package — during the mirror's object phase, in the object half of an upload, inside a move's object loop, each of them before the ordering rule below applies. For the served and legacy names the first path segment decides it, after `./` is dropped and with case folded, because the build tree and a local backend are filesystems: one name is a file or a directory and not both, and APFS answers `Data.pkg` with `data.pkg`. A fallback name is refused only as the whole path, case folded: nothing is ever written under it, so `data.tzst/example.pkg` collides with no file and mirrors, uploads and appears in a generated catalogue like any other package. A package at the repository root is an ordinary layout and stays mirrored; only these names are not its to take.
...
- A **hosted** entry's catalogue is never fetched from upstream on a miss, under the names bodega knows (see [What is deliberately not served](#what-is-deliberately-not-served) for a `meta.conf` that renames them). Upstream's catalogue is by construction newer than this mirror's objects and names packages the store has never held, so a miss answers 404 and `pkg` reports the repository as unavailable rather than installing half a transaction. Objects on a hosted entry may still be proxied: an object arriving late can only complete an install, never break one.
...
A hosted entry, mirrored or generated, also refuses the names pkg falls back to when the served three 404: `meta.txz`, and `data` or `packagesite` under a `packing_format` extension (`data.tzst`, `packagesite.tgz` and so on, `packagesite.txz` already being on the list above). pkg 2.7.5 asks for `meta.conf` then `meta.txz`, and for each archive `<name>.pkg` then `<name>.<packing_format>`, so a client in front of a mirror that has not finished asks for all of them. Fetching one would serve upstream's catalogue over this mirror's objects, and a repository built by pkg 1.17 through 1.20 does publish the `.tzst` pair. A `proxy` entry holds no catalogue of its own, so there these names are fetched like any other path.

Those are pkg's default names, and the list stops there. pkg 2.7.5 (`libpkg/pkg_repo.c`, `pkg_repo_fetch_extract_to_fd` and `pkg_repo_fetch_remote_tmp`) asks for each catalogue archive by the `data` and `manifests` keys of the `meta.conf` it read, up to the last dot, as `<name>.pkg` then `<name>.<packing_format>`. A generated entry writes the defaults. A mirrored entry serves upstream's `meta.conf` as stored, so a mirror of a repository whose `meta.conf` sets `data = "custom"` sends its clients to `custom.pkg` and `custom.<packing_format>`, which no list guards: with `proxy_cache_enabled: true` both are fetched from upstream, disclosing the ABI and repository, and served if upstream has them. No `pkg.FreeBSD.org` repository renames either archive. Refusing those names would mean reading them out of the stored `meta.conf` per request, which bodega does not do.
...
A refusal like this one goes to the log at error, beside the repository's ABI, name and requested file. The client gets a 500 whose body says only that the repository is not serving a catalogue right now and that retrying is safe. The reason stays out of the body because pkg reads this route without a token, and the same failure path can name the signing key's path or the storage root.
...
Every `bodega freebsd key` subcommand takes `--path` to write or read one named file instead of searching the server's paths, and `export --fingerprint` emits the trusted-fingerprint file rather than the public key. `pkgrepo` is accepted as an alias for `freebsd`.
...
A version entry's `url` may carry userinfo, `https://user:secret@host/path`, or a token in its query string, `https://host/path?token=secret`, and the builder and the proxy routes fetch with it as written: Go's HTTP client and `git` send the userinfo as basic auth, and the query goes to the upstream with the request. bodega has no other place to configure a credential per upstream, so both forms are supported rather than refused.

The four `GET /api/v1/packages` routes answer every caller, so they publish every `url` with the userinfo, the query and the fragment removed, username included, whatever the caller's address or token: `https://audit-user:audit-secret@private.example/latest` is served as `https://private.example/latest`, `//audit-user@private.example/latest` as `//private.example/latest`, and the git forms `audit-user#audit-secret@private.example:repo.git` and `ssh://audit-user#audit-secret@private.example/repo.git` as `private.example:repo.git` and `ssh://private.example/repo.git`, since git hands ssh everything before the host as the username, and `https://private.example/latest?token=secret#frag` as `https://private.example/latest`. Where readers disagree about where the authority ends, the cut takes the widest reading, so a url with no path whose query or fragment holds an `@` is published shorter than it is. The query goes whole rather than by parameter name, because nothing about `?token=`, `?access_token=`, `?X-Amz-Signature=` or the next spelling says which values are secret; a git scp path holding a `?` or `#` is published cut there. The username goes too because a GitHub or GitLab token written as `https://<token>@host/` is a bare username, and masking the password alone would publish it whole. The web UI reads these routes, so it shows the same form. The stored manifest is untouched, and `bodega show pkg`, `bodega pkg export`, `bodega pkg edit`, the TUI and the builder's output, which read the store on the server host rather than the API, still print the url as written.

`freebsd.refused[].error` on `/api/v1/status` quotes an entry's url, so it is empty for a caller outside `admin_permit_cidr`. `repo` and `abi` stay public on every row, so a client can still tell a refused repository from an absent one. Inside `admin_permit_cidr` the error is returned in full with no token, and so is the before and after manifest JSON on `/api/v1/audit`: on reads, the admin range is the boundary for the stored credential. See [Threat model](threat-model.md) for every surface and the boundary it sits behind.

A binary entry with no `filename` is stored under its url's last segment. For a url with no path, that segment is the authority, so the object key is `binaries/<name>/<version>/user:secret@host`. The read routes publish every binary entry with a `filename` of `~/<tag>/<display>`, and the web UI links to it as `/binaries/<name>/<version>/~/<tag>/<display>`. Every entry gets one, not only those whose stored name carries userinfo: a request for a stored name is served from the backend of the first entry of its version, so an entry of the same version and name added later on another backend would take over a link built from the stored name. `<display>` comes from the explicit `filename` or the published url, never the stored url, so no spelling of the url puts its userinfo back, and it takes no part in the lookup. The tag is 32 hex digits of an HMAC over the package name, the version, the storage backend the entry records and the stored name, keyed by the server's token pepper, so it depends on that entry alone and nobody without the pepper can derive it or test a guessed credential against it. A server with no pepper cannot mint a tag that resolves, and a stored name is served from whichever entry of its version comes first, so a link built from one would pass to an entry later added ahead of it. It publishes no download link instead: every binary entry's `filename` is the withheld alias, `~/00000000000000000000000000000000/<display>`, which answers 404, and the web UI shows no download link for it. The server logs `could not load or create pepper file` at startup when it is in this state.

`/binaries/` reads a request as an alias by its segments alone: `~` after the version (after the name, for an entry with no version), then the tag and the display name. An alias reaches the entry of that version its tag identifies and is served from the backend that entry records, or answers 404; it is never looked up as a stored name, and a stored name, which is one segment, is never looked up as an alias. A saved alias link therefore keeps reaching the same entry, or answers 404 once that entry is gone, whatever entries are later added, removed or reordered, including one given the alias as an explicit `filename` and one holding the same object key on another backend. Moving the entry to another backend with `bodega pkg move` retires its alias the same way: the saved link answers 404 and the read routes publish a new one. Rotating the pepper, or running with none, turns every published alias into a 404 and never into a request for the object stored under that spelling. Every other name is served as stored, whatever it looks like, so links saved before this change keep working. The TUI links an entry by the same alias, which it mints with the pepper it finds at the same paths the server reads. A host that cannot read the pepper gets the stored name when the route serves that name from the entry's own backend, which for a path that names no version of the package means the backend `storage_by_type.binary` selects, and no link otherwise rather than a link to other bytes. For an entry with no version, the TUI's link now leaves out the empty version segment instead of relying on the server's redirect from `//`. The key itself still carries the credential, so anyone who can list the storage backend reads it. Set `filename` on such an entry and fetch it again to store the object under a name that carries nothing.

**Upgrading.** Nothing refuses to start and no entry is refused: a manifest that already carries a credential keeps serving and fetching as before, and only what the read API and the status refusal publish changes. A binary download at its stored name still answers as it did, userinfo in the filename or not, so a saved link keeps working, with the same limit it always had: it reaches the first entry of its version, which is not always the entry it was saved for. A server that finds no pepper publishes no binary download link at all, on the read routes or in the web UI, until the pepper is readable at `/etc/bodega/pepper` or `~/.config/bodega/pepper`; fix the pepper rather than building links from the stored name. The read routes now publish every binary entry's `filename` as its alias, so the web UI and the TUI print alias links for every binary entry, and a client that reads `filename` from the API to build its own download URL gets one too. Three consequences follow:

- A manifest fetched from the read API and pushed back through `bodega pkg import` or `POST /api/v1/packages/{type}` arrives without its credential, and the next fetch from that upstream goes out anonymous. A binary entry also arrives carrying the published `filename`, `~/<tag>/<display>`, so its next fetch stores a new object under that name. The server reads that spelling as the original entry's alias, so the new entry is reachable only through the alias the read routes publish for it in turn, and the TUI links it by that alias too, or shows no link on a host that cannot read the pepper. Import from the manifest file under `manifest_dir` instead.
...
- A client that built a binary download URL from a url it read before this change keeps a link carrying the credential. It still works, which is the reason to rotate rather than rely on the new link.

`metadata` is public by contract. bodega never fetches with a metadata value, so it has no use for a credential in one, and every value goes out on the read routes and, for an apt entry, in the `Packages` stanza. The one exception is a value written as a URL: a scheme followed by `/` or `\`, a scheme of `http` or `https`, or a scheme-relative `//host`. Those surfaces publish it through the same cut as `url`, so `homepage: https://user:secret@docs.example/p?token=secret` is served as `https://docs.example/p`, under whatever key it sits. Text that is not a URL is published as written, `@`, `?` and `#` included: `Jane Doe <jane@example.org>` and `mailto:jane@example.org` are unchanged. The test is on the whole value, so a URL inside longer text, `see https://user:secret@host/`, and a schemeless `user:secret@host/x` are published as written too. A secret that is not part of a URL, `api_token: secret`, is published as written to every caller. Keep it out of the manifest.

An apt entry's `Architecture`, `_pool_path`, `_md5`, `_sha1` and `_sha256` do not take a URL with a credential either, because the index publishes them as what they are rather than as text: `Architecture` names the index paths and the `Architectures` line in `Release` and the signed `InRelease`, `_pool_path` becomes the `Filename` apt fetches, and the digests are what apt checks the bytes against. Cutting the credential out would rename an index or publish a checksum nothing matches. `bodega pkg import`, `bodega pkg edit` and `POST /api/v1/packages` refuse an apt entry whose value for one of those keys is a URL carrying userinfo, a query or a fragment, and name the key in the refusal. An entry already stored with one reaches no `Packages` index and adds no architecture to `Release`, and every rebuild logs `apt entries carry a URL with userinfo, a query or a fragment in an architecture, pool path or digest` with the package, version and key, never the value. The pool object itself is still served at its path.
...
- `host` is the one stanza the host that asked should install, so a client installs it rather than picking a block out of `sources`. `profile` names the profile that scoped apt for that host, and is absent when none does — an instance serving a single codename answers `host` for every caller whose profile does not scope apt. `host` is absent in two states. A profile that scopes apt with no codename generated for it yet answers `profile` and no `host`: the rebuild refused the base or has not run since the profile was written, and naming the base instead would serve the unfiltered index the profile exists to narrow. Check the server's `ERROR` logs for profile refusals, upstream read failures, or a failed rebuild; a rebuild can fail before it evaluates profiles. Both are absent when the answer is the operator's: no profile scopes apt and this instance serves more than one codename, and a guess there points a host at the wrong suite with nothing reporting it. `bodega doctor --write-apt-sources --suite <codename>` is the operator giving that answer, and it installs the block `sources` carries for that codename.
...
`bodega shell` launches a full-screen terminal interface with three panes:

- **Sources**, top left: a tree of every manifest, grouped by type, then package, then version. `/` filters it, and the pane title shows the query.
- **Details**, top right: the selected entry's name, ref, checksum, where its bytes are stored, its dependencies and dependents, the client instruction for its type, and the raw JSON. See [Details pane](#details-pane).
- **Log**, across the bottom: the output of every action the TUI runs, with the session log's path in the pane title.

`Tab` moves focus from Sources to Details to Log and back, and the focused pane has a highlighted border. A popup (help, a form, a confirmation, the token manager, the JSON editor, the audit table) replaces the whole screen until it closes, rather than floating over the panes.
...
make depend         # install Go + golangci-lint (downloads the linux-amd64 toolchain)
```
