...
A binary entry with no `filename` is stored under its url's last segment. For a url with no path, `https://user:secret@host`, that segment is the authority, so the object key is `binaries/<name>/<version>/user:secret@host`. The read routes publish such an entry with a `filename` of `<host>-<tag>`, and the web UI links to it: the tag is the first 12 characters of the entry's `artifact_digest`, or its 1-based position in `versions` when it has never been fetched, and a further `-2`, `-3` is added when another entry of that version is already stored under or published as the name. `/binaries/` serves that name from the entry's own stored key. A name some entry is stored under always serves that entry, so an entry whose explicit `filename` is `host` keeps `host`. A link read before the manifest changed reaches the same bytes or a 404, except for an entry never fetched, whose position shifts when an earlier entry is removed. The key itself still carries the credential, so anyone who can list the storage backend reads it. Set `filename` on such an entry and fetch it again to store the object under a name that carries nothing.
