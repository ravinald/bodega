...
**`pkg bootstrap` is the one thing this does not give you, on a hosted mirror, and often not on a proxied one either.** pkg's bootstrapper fetches `<repo>/Latest/pkg.pkg` and `<repo>/Latest/pkg.pkg.sig` and nothing else, and a hosted mirror publishes only the repopaths its catalogue names, which never include that pair. A repository in `mode: proxy` composes an upstream URL for any path outside the catalogue, so the two paths resolve against upstream rather than against bodega's catalogue. Upstream publishes a pkg package under the ports repositories and nowhere else. Measured with `fetch` against `pkg.FreeBSD.org` on 2026-09-21, both paths per repository: `latest` and `quarterly` answer 200 on `FreeBSD:15:aarch64` and `FreeBSD:14:amd64`; every `base_*` and `kmods_*` repository answers 403 or 404 on both, including `base_release_0`, which serves `pkg.pkg` and 403s the signature the bootstrapper checks it against; `release_0` and `release_1` answer 200 on 15 and 404 on 14. What the client sees is not always what upstream said: bodega passes a 404 through and turns any other upstream refusal into a 502 whose body names nothing, with the refusing URL in the server log. So bodega renders three answers rather than two, and a repository nobody measured gets the hedge: the emitted comment says which case this file is, and the `bootstrap` field on `GET /api/v1/status` says the same in one word. Install pkg from upstream, or from a ports repository through bodega, before switching a host over.
