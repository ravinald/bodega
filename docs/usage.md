...
- A restricted port restricts the names in every `distinfo` its fetch may check against: `${DISTINFO_FILE}`, which defaults to `${MASTERDIR}/distinfo`, resolved from the same variables at each framework include and at the end of the read. So a slave restricts its master's names however it spells `MASTERDIR` (`${.CURDIR}/..`, `${.CURDIR:H:H}/...`, `${PORTSDIR}/...`, in an included file, or on either side of an `.if`), and a port pointing `DISTINFO_FILE` at another port's directory restricts that one. Only the directory has to resolve: every `distinfo*` in it is restricted, so `distinfo.${ARCH}` is covered.
- Where that directory cannot be resolved, the port's own directory is still restricted, but names it reads from another port's `distinfo` are not, because nothing short of `make` says which port that is. On the 15.1 test tree that is 111 restricted ports: 110 whose scan stopped at an include it could not follow, and 1 whose `DISTINFO_FILE` holds a modifier the scan does not read. `bodega serve` logs them after every read of the tree (`distinfo: N restricted ports read a distinfo bodega cannot place`), and `build fetch distfiles` and a distfiles upload print the same list as a warning, each port with its restriction and the reason.
