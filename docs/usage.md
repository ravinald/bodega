...
`binary`, `git`, `helm` and `freebsd` have no OSV identifier. `set` refuses them:
...
`apt`, `binary`, `git`, `helm` and `freebsd` have no such source. `set` refuses them:
...
The empty declaration is the default, and it refuses every distfile on a stock tree: 139 ports read something outside the tree or a variable only the client sets, and their `distinfo` cannot be placed. This is the declaration the 15.1 test tree was measured with, for a stock `arm64` client with no aspell installed and no `/etc/make.conf` beyond the lines [The client check](#the-client-check) adds:
...
- Every makefile `make` reads after `make.conf` is under `/usr/share/mk`, under `${PORTSDIR}`, relative to the port's directory, or declared. A `-f` file, and a saved options file under `/var/db/ports` that nobody declared, names `unsupported`.
