...
- Quoted `.include` lines are followed, from the Makefile and every file it reaches, so a `NO_CDROM` in an included file counts. Lines are read in make's order, and a path is resolved from `${.CURDIR}`, `${.PARSEDIR}`, `${PORTSDIR}`, the `:H` modifier and the variables the port assigns. `:=` takes its value where it stands, a file included twice is read twice with the values in force at each include, and `MASTERDIR` and `FILESDIR` take the framework's defaults at the framework include. Conditionals are not evaluated: an assignment under `.if`, or in a file included under one, adds a possible value, and every value's file is read. A variable no assignment has set is undefined, and one make may skip (under `.if`, in a file included under one, or in a `.for` whose list may be empty) leaves it possibly undefined where nothing set it before, as does an `.undef` that may not run. A path reading a possibly undefined variable refuses the port, because make expands it to nothing where the scan sees only the assigned value; a `?=` assigns only on the paths where it may be undefined. A `.for` whose list cannot be empty runs its body at least once, so an assignment there is not one make may skip. A `.for` variable takes every word of the loop's list, and is substituted into the loop's own text as make does, so inside the loop it shadows a variable of the same name and outside it does not. A path refuses the port when it does not resolve, reads a variable set by `!=`, by `+=` or `:=` inside `.for`, through a variable-built name that does not resolve, or by a `::=` or `:_=` modifier, leaves the ports tree, includes the file that includes it, reaches more than 256 file reads, or names a missing file under a plain `.include`; a missing `.sinclude` or `.-include`, or any include under a conditional, does not. Angle-bracket includes and anything under `Mk/` are the framework and are not read.
...
- **On a stock ports tree this refuses everything.** The 15.1 test tree has 113 such ports: 110 whose scan stopped at an include it could not follow, and 3 whose `DISTINFO_FILE` holds a value the scan cannot resolve (`distinfo.${ARCH}`, `distinfo-${ARCH}`, `${PKGNAMESUFFIX}`). 91 of the 110 are the aspell dictionaries, whose shared `textproc/aspell/Makefile.inc` includes `${LOCALBASE}/etc/aspell.ver` when the client host has one. That file sits outside the ports tree, on whichever host runs `make`, and base make lets it move `DISTINFO_FILE` and set `NO_CDROM`, so no reading of the tree (`make` on the bodega host included) can place those ports. Serving distfiles from a stock tree needs a decision about files outside the tree that bodega has not taken yet.
