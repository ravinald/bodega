...
It then checks every binary version's `filename` against the rule under [Binary-specific fields](#binary-specific-fields) and prints an `INVALID` row, with the reason, for each one that breaks it. Any `INVALID` row makes the command exit non-zero. Fix one by correcting `filename` or removing it with `bodega pkg edit`.
...
- **filename**: overrides the basename derived from the URL. It must be a clean relative path: subdirectories such as `~/tool` are allowed, but not a `..` or `.` segment, an empty segment (`a//b`, a trailing `/`), a leading `/`, a `\` or a NUL. The fetch joins it into the build root and the upload joins it into the object key, so a parent reference or an absolute path would move a write. Every manifest write refuses one that breaks the rule (`pkg create`, `pkg edit`, `pkg import`, the mutation API, and a write the builder makes after a fetch), and so does the fetch.
  ...
  A manifest already carrying one, written before the check or edited by hand, still loads: `bodega serve` starts and the package's other versions serve as before. That version's fetch fails with an error naming the filename, its upload is skipped, its download alias answers 404, and `pkg delete`, `pkg move` and `repair keys` stop at it rather than derive an object key whose `..` could land on another package's object. Any write of the manifest is refused until the filename is corrected or removed, including the checksum a fetch of a sibling version records. Run `bodega pkg verify` after upgrading to list them before a fetch finds them.

  The fetch applies the same rule to the rest of the destination. The version must be one directory, and the package name must be a clean relative path, so neither can move the file outside `binaries/<name>/<version>/`. The write refuses a destination whose directory resolves through a symlink to outside the build root, or that is a symlink itself. A build root that is a symlink is fine.
...
The four `GET /api/v1/packages` routes answer every caller, so they publish every `url` with the userinfo removed, username included, whatever the caller's address or token: `https://audit-user:audit-secret@private.example/latest` is served as `https://private.example/latest`, `//audit-user@private.example/latest` as `//private.example/latest`, and the git forms `audit-user#audit-secret@private.example:repo.git` and `ssh://audit-user#audit-secret@private.example/repo.git` as `private.example:repo.git` and `ssh://private.example/repo.git`, since git hands ssh everything before the host as the username. Where readers disagree about where the authority ends, the cut takes the widest reading, so a url with no path whose query or fragment holds an `@` is published shorter than it is. The username goes too because a GitHub or GitLab token written as `https://<token>@host/` is a bare username, and masking the password alone would publish it whole. The web UI reads these routes, so it shows the same form. The stored manifest is untouched, and `bodega show pkg`, `bodega pkg export`, `bodega pkg edit`, the TUI and the builder's output, which read the store on the server host rather than the API, still print the url as written.
