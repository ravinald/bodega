...
| the type marker | `profile_types`   | membership (`closed`, `open`) and the version default (`pinned`, `floating`) for one package type |
...
`constraint_kind` holds the four values already on `manifest.VersionEntry`: `exact`, `compatible`, `patch`, `any`. A fifth spelling of the same idea is a defect waiting for the day the two disagree about what `^` matches. Empty is the third level declining to override, which defers to the type's default.
...
**None of this is enforced on the serve path yet.** `014` builds the model, the storage, the commands and the predicate; the read path does not consult a profile, so what bodega answers today is what it answered before. `bodega pin` is the host-side half of the same idea and is a different thing: it emits apt preferences for a host to apply, where a profile decides what bodega will answer.
