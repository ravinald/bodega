...
**apt is excluded on purpose.** Refusing an apt fetch at the pool leaves `dpkg` holding a half-configured transaction, which is a worse outcome than no control at all, so the apt answer belongs at the generated index and is tracked separately.

**A filtered index carries the profile in its cache key.** Cached under the shared key it would be served to every other host class, with no error anywhere: the second host gets a document that is valid, parseable and wrong about what it may install. `profileIndexKey` prefixes `profiles/<profile>/`, deliberately outside every type tree so `manifest.ParseKey` cannot read it as one of the eight — the checksum table is keyed by object key (`011`), and the same bytes acquiring a second identity is what this design refused to do at the storage level. Indexes are mutable and so are never checksummed, which is what makes the scoped key safe rather than merely unobserved. An unidentified request, and an identified one whose profile states no rule for the type, both keep today's key: an install with no profiles pays nothing. The filter itself runs on the way out, over the buffered response, for the reason the pypi href rewrite and the npm tarball rewrite do — the cached object stays the document the upstream served, and a cache hit filters identically to a miss, so an operator's profile edit lands within the binding cache TTL rather than at the next refresh.
