...
func ScanWheelMetadata(wheelDir string, store *manifest.Store) (*PypiDepGraph, error) {
	...
	for _, whlPath := range matches {
		...
		key := manifest.CanonicalPypiName(info.Name)
	...
	for _, name := range store.ListPackages(manifest.TypePypi) {
		key := manifest.CanonicalPypiName(name)
		...
		for _, ve := range pm.Versions {
			for _, requiredBy := range ve.RequiredBy {
				baseApps[manifest.CanonicalPypiName(requiredBy)] = true
			}
		}
	}
	...
	for key, pkg := range graph.Packages {
		for _, req := range pkg.Requires {
			depName := manifest.CanonicalPypiName(pkgBaseName(req))
