...
// A binary paragraph names its source's version only when the two differ, so
// both halves need covering: a caller that closed its membership over the
// source name has to compare the source's version, and reading Version: off
// the paragraph is right on every stanza except the one that matters.
func TestSourceVersion(t *testing.T) {
	cases := []struct {
		...
		{
			name:   "a binNMU names the source version it was rebuilt from",
			fields: map[string]string{"Package": "nginx-common", "Source": "nginx (1.24.0-2ubuntu7.1)", "Version": "1.24.0-2ubuntu7.1+b1"},
			want:   "1.24.0-2ubuntu7.1",
		},
		{
			name:   "a bare Source field falls through to the binary's own version",
			fields: map[string]string{"Package": "nginx-common", "Source": "nginx", "Version": "1.24.0-2ubuntu7.1"},
			want:   "1.24.0-2ubuntu7.1",
		},
		{
			name:   "no Source field at all",
			fields: map[string]string{"Package": "nginx", "Version": "1.24.0-2ubuntu7.1"},
			want:   "1.24.0-2ubuntu7.1",
		},
		{
			name:   "empty parentheses are not a version",
			fields: map[string]string{"Package": "nginx-common", "Source": "nginx ()", "Version": "1.24.0-2ubuntu7.1+b1"},
			want:   "1.24.0-2ubuntu7.1+b1",
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := SourceVersion(tc.fields); got != tc.want {
				t.Errorf("SourceVersion(%v) = %q, want %q", tc.fields, got, tc.want)
			}
		})
	}
}
