...
// The json tags are the wire shape of `bodega discover export json`, and they
// are the CSV header of `bodega discover export csv` field for field. Untagged,
// the two formats of one command disagreed: CSV emitted pkg_name and JSON
// emitted PkgName, so a consumer written against the column names in
// docs/usage.md read nothing out of either format.
type DiscoveryRow struct {
	RegistryType string    `json:"registry_type"`
	Host         string    `json:"host"`
	PatternHint  string    `json:"pattern_hint"` // suggested promotion pattern (policy.SuggestPattern)
	PkgName      string    `json:"pkg_name"`
	PkgVersion   string    `json:"pkg_version"`
	Decision     string    `json:"decision"`     // one of the Decision* constants above
	UpstreamURL  string    `json:"upstream_url"` // full upstream URL bodega fetched (or would have); empty on rows recorded before migration 007
