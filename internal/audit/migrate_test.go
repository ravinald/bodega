...
func TestMigration009PreservesRowsAndIndexes(t *testing.T) {
	...
	if _, err := raw.Exec(
		`INSERT INTO upstream_discovery (registry_type, pattern_hint, pkg_name, pkg_version, decision)
		 VALUES ('gomod','example.com/example-corp/','example.com/example-corp/widget-sdk','v1.30.0','no_manifest')`,
	); err != nil {
...
func TestMigration010RelabelsWouldDeny(t *testing.T) {
	...
	const seed = `INSERT INTO upstream_discovery
	 ...
	 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	for _, row := range [][]any{
		{"gomod", "example.com/example-corp/", "example.com/example-corp/widget-sdk", "v1.30.0", "denied",
			"2026-01-02T00:00:00.000Z", "2026-01-02T00:00:00.000Z", "10.0.0.2", 3, "https://proxy.golang.org/x"},
		{"gomod", "example.com/example-corp/", "example.com/example-corp/widget-sdk", "v1.30.0", "would_deny",
...
var preIdentityRows = []struct {
	...
	{"pypi/wheels/1.26.0/examplesdk-1.26.0-py3-none-any.whl", "pypi", "1.26.0/examplesdk", "1.26.0"},
	{"binaries/example-tool/2.15.0/example-toolv2.zip", "binary", "example-tool", "2.15.0"},
}
...
func TestOpenBackfillsChecksumIdentityOnUpgrade(t *testing.T) {
	...
	for _, tc := range []struct{ typ, name, verz string }{
		{manifest.TypeGomod, "example.com/example-corp/widget-sdk", "v1.30.0"},
		...
		{manifest.TypePypi, "examplesdk", "1.26.0"},
		{manifest.TypeBinary, "example-tool", "2.15.0"},
	} {
