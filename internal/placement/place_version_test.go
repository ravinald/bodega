...
func recordedStorage(t *testing.T, store *manifest.Store) string {
	t.Helper()
	pm, err := store.GetPackage(t.Context(), manifest.TypeBinary, "example-tool-v2")
	if err != nil || pm == nil {
...
func TestPlaceVersionBeatsTheRecordedNameAndTheRule(t *testing.T) {
	...
	pl.PlaceVersion("example-tool-v2", "2.15.0", "bulk")

	st, err := pl.ForVersion(t.Context(), manifest.TypeBinary, "example-tool-v2", "2.15.0", "binaries/example-tool-v2/2.15.0/example-tool")
	if err != nil {
...
func TestPlaceVersionRecordsSoTheNextUploadFollowsWithoutTheFlag(t *testing.T) {
	...
	pl.PlaceVersion("example-tool-v2", "2.15.0", "bulk")
	if _, err := pl.ForVersion(t.Context(), manifest.TypeBinary, "example-tool-v2", "2.15.0", "k"); err != nil {
		t.Fatalf("ForVersion: %v", err)
	}

	next := NewWith(pl.Stores(), store, io.Discard, false)
	if _, err := next.ForVersion(t.Context(), manifest.TypeBinary, "example-tool-v2", "2.15.0", "k"); err != nil {
...
func TestPlacedVersionIsFalseWhenTheVersionWasNeverReached(t *testing.T) {
	pl, store := placerFixture(t, "")
	pl.PlaceVersion("example-tool-v2", "2.15.1", "bulk") // one digit off
...
func TestPlaceVersionMatchesARefAsWellAsAVersion(t *testing.T) {
	...
	store := manifest.NewLocalStore(t.TempDir())
	if err := store.AddVersion(t.Context(), manifest.TypeGit, "widget", manifest.VersionEntry{Ref: "v4.5.5"}); err != nil {
		t.Fatalf("AddVersion: %v", err)
	}
	pl := NewWith(stores, store, io.Discard, false)
	pl.PlaceVersion("widget", "v4.5.5", "bulk")

	if _, err := pl.ForVersion(t.Context(), manifest.TypeGit, "widget", "v4.5.5", "repos/widget/v4.5.5.bundle"); err != nil {
		t.Fatalf("ForVersion: %v", err)
	}
	if !pl.PlacedVersion() {
