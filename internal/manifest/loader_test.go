...
func TestStore_GetPackage_HappyPath(t *testing.T) {
	...
	writePackageManifest(t, dir, manifest.PackageManifest{
		...
		Versions: []manifest.VersionEntry{
			{Ref: "v4.2.0", URL: "https://example.com/example-corp/widget"},
		},
	})

	store := manifest.NewLocalStore(dir)
	pm, err := store.GetPackage(ctx, manifest.TypeGit, "widget")
	if err != nil {
	...
	if pm.Name != "widget" {
		t.Errorf("expected name=widget, got %q", pm.Name)
	}
...
func TestStore_RemoveVersion(t *testing.T) {
	...
	store := manifest.NewLocalStore(dir)
	if err := store.AddVersion(ctx, manifest.TypeGomod, "example.com/example-corp/widget-sdk", manifest.VersionEntry{Version: "v1.30.0"}); err != nil {
		t.Fatalf("AddVersion: %v", err)
	}
	if err := store.AddVersion(ctx, manifest.TypeGomod, "example.com/example-corp/widget-sdk", manifest.VersionEntry{Version: "v1.31.0"}); err != nil {
		t.Fatalf("AddVersion v1.31.0: %v", err)
	}

	if err := store.RemoveVersion(ctx, manifest.TypeGomod, "example.com/example-corp/widget-sdk", "v1.30.0"); err != nil {
		t.Fatalf("RemoveVersion: %v", err)
	}

	pm, err := store.GetPackage(ctx, manifest.TypeGomod, "example.com/example-corp/widget-sdk")
	if err != nil {
		t.Fatalf("GetPackage: %v", err)
	}
...
func TestStore_Index_LoadSave(t *testing.T) {
	...
	writeIndex(t, dir, manifest.Index{
		Packages: map[string][]string{
			manifest.TypeGit: {"widget", "myrepo"},
...
func TestStore_Graph_AddEdgeAndQuery(t *testing.T) {
	...
	store.AddEdge(manifest.DepEdge{Parent: "git/widget", Child: "pypi/django"})
	store.AddEdge(manifest.DepEdge{Parent: "git/widget", Child: "pypi/requests"})
	store.AddEdge(manifest.DepEdge{Parent: "git/other", Child: "pypi/requests"})

	children := store.ChildrenOf("git/widget")
	if len(children) != 2 {
		t.Errorf("expected 2 children of git/widget, got %+v", children)
	}
...
func TestStore_Graph_RemoveEdge(t *testing.T) {
	...
	store.RemoveEdge("git/widget", "pypi/django")
...
func TestStore_Graph_Deduplication(t *testing.T) {
	...
	store := manifest.NewLocalStore(dir)
	e := manifest.DepEdge{Parent: "git/widget", Child: "pypi/django"}
	store.AddEdge(e)
	store.AddEdge(e)
	store.AddEdge(e)

	if got := store.ChildrenOf("git/widget"); len(got) != 1 {
...
func TestStore_Graph_SaveAndLoad(t *testing.T) {
	...
	if got := store2.ChildrenOf("git/widget"); len(got) != 1 || got[0].Child != "pypi/django" {
...
func TestStore_Graph_Orphans(t *testing.T) {
	...
	store := manifest.NewLocalStore(dir)
	// Create "git/myapp" in the store but NOT "pypi/examplesdk".
	// examplesdk is referenced in the graph but has no manifest — it's an orphan.
	_ = store.AddVersion(ctx, "git", "myapp", manifest.VersionEntry{Ref: "main"})
	store.AddEdge(manifest.DepEdge{Parent: "git/myapp", Child: "pypi/examplesdk"})

	orphans := store.Orphans()
	if len(orphans) != 1 || orphans[0] != "pypi/examplesdk" {
		t.Errorf("expected [pypi/examplesdk] as orphan, got %v", orphans)
	}

	// Now add examplesdk to the store — no more orphans.
	_ = store.AddVersion(ctx, "pypi", "examplesdk", manifest.VersionEntry{Version: "1.0"})
	orphans = store.Orphans()
	if len(orphans) != 0 {
		t.Errorf("expected no orphans after adding examplesdk, got %v", orphans)
	}
}

func TestStore_SafeName_SlashEncoding(t *testing.T) {
	safe := manifest.SafeName("example.com/example-corp/widget-sdk")
	if safe != "example.com--example-corp--widget-sdk" {
