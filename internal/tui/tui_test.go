...
func TestSplitArgs(t *testing.T) {
	tests := []struct {
		...
		{"freeze binary/example-tool-v2", []string{"freeze", "binary/example-tool-v2"}},
	}
...
func TestExtractFlag(t *testing.T) {
	args := []string{"git", "--entry", "widget", "pypi"}
	val, remaining := extractFlag(args, "--entry")
	if val != "widget" {
		t.Errorf("value = %q, want widget", val)
	}
...
func TestObjectKeySafeEncoding(t *testing.T) {
	store := manifest.NewLocalStore(t.TempDir())
	ctx := t.Context()
	_ = store.AddVersion(ctx, manifest.TypeNpm, "@example-corp/widget-cli", manifest.VersionEntry{Version: "1.4.2"})
	_ = store.AddVersion(ctx, manifest.TypeGomod, "example.com/example-corp/widget-sdk", manifest.VersionEntry{Version: "v1.2.3"})
	_ = store.AddVersion(ctx, manifest.TypeGit, "example-corp/widget", manifest.VersionEntry{Ref: "v4.5.7"})

	cases := []struct {
		label, typ, name, want string
	}{
		{"npm scoped", manifest.TypeNpm, "@example-corp/widget-cli", "npm/@example-corp--widget-cli/@example-corp--widget-cli-1.4.2.tgz"},
		{"gomod keeps its slashes", manifest.TypeGomod, "example.com/example-corp/widget-sdk", "gomod/example.com/example-corp/widget-sdk/@v/v1.2.3.zip"},
	}
	for _, c := range cases {
	...
	gitPath := objectKey(store, manifest.TypeGit, "example-corp/widget", "")
	if strings.Contains(gitPath, "example-corp/widget") {
...
func TestExtractNameFromURL(t *testing.T) {
	cases := []struct {
		...
		{"https://github.com/org/widget.git", manifest.TypeGit, "org/widget"},
		{"https://github.com/org/repo", manifest.TypeGit, "org/repo"},
		{"https://example.com/downloads/example-tool-2.0.tar.gz", manifest.TypeBinary, "example-tool-2.0"},
