...
func TestTUIUploadReachesTheNamedBackend(t *testing.T) {
	...
	if err := store.AddVersion(t.Context(), manifest.TypeBinary, "example-tool", manifest.VersionEntry{
		Version: "2.0.0", URL: "https://example.com/example-tool.zip",
	}); err != nil {
		t.Fatalf("AddVersion: %v", err)
	}
	seedLocal(t, cfg.BuildRoot, "binaries/example-tool/2.0.0/example-tool.zip", "binary-bytes")
	...
	key := manifest.BinaryKey("example-tool", "2.0.0", "example-tool.zip")
...
func TestTUIUploadCoversEveryType(t *testing.T) {
	...
	seeds := []struct {
		...
		{manifest.TypeGomod, "example.com/example-corp/widget-sdk", "gomod/example.com--example-corp--widget-sdk/@v/v1.30.0.zip",
			manifest.VersionEntry{Version: "v1.30.0"}, manifest.GomodKey("example.com/example-corp/widget-sdk", "v1.30.0", ".zip")},
		...
		{manifest.TypeNpm, "@example-corp/widget-cli", "npm/@example-corp--widget-cli/1.5.0/@example-corp--widget-cli-1.5.0.tgz",
			manifest.VersionEntry{Version: "1.5.0"}, manifest.NpmTarballKey("@example-corp/widget-cli", "1.5.0")},
...
func TestTUIRemoveReachesTheRecordedBackend(t *testing.T) {
	...
	if err := store.AddVersion(t.Context(), manifest.TypeBinary, "example-tool", manifest.VersionEntry{
		Version: "2.0.0", URL: "https://example.com/example-tool.zip", Storage: "bulk",
	}); err != nil {
		t.Fatalf("AddVersion: %v", err)
	}
	key := manifest.BinaryKey("example-tool", "2.0.0", "example-tool.zip")
	...
	var buf bytes.Buffer
	if err := runRemove(&buf, nil, store, stores, manifest.TypeBinary, "example-tool"); err != nil {
...
func TestTUIRemoveRefusesWhenNoKeyResolves(t *testing.T) {
	...
	var buf bytes.Buffer
	err := runRemove(&buf, nil, store, stores, manifest.TypePypi, "examplesdk")
	if err == nil {
...
func TestTUIMutationsSignalTheServer(t *testing.T) {
	...
	seedLocal(t, bulkRoot, manifest.BinaryKey("example-tool", "2.0.0", "example-tool.zip"), "binary-bytes")
	...
	verbs := []struct {
		...
		{"remove", func(b *bytes.Buffer) error {
			return runRemove(b, cfg, store, stores, manifest.TypeBinary, "example-tool")
		}},
		{"freeze", func(b *bytes.Buffer) error {
			return runFreeze(b, cfg, store, manifest.TypeBinary, "example-tool", nil)
		}},
		{"unfreeze", func(b *bytes.Buffer) error {
			return runFreeze(b, cfg, store, manifest.TypeBinary, "example-tool", nil)
		}},
		{"delete", func(b *bytes.Buffer) error {
			return runDelete(b, cfg, store, manifest.TypeBinary, "example-tool", nil)
		}},
	}
	for _, v := range verbs {
		var buf bytes.Buffer
