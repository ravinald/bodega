...
// manifest. Save path is shared with pkg edit CLI and the web API.
...
func (m *appModel) makeEditSave(entryType, entryName, editVersion string) func(buf []byte) error {
	return func(buf []byte) error {
		...
		if editVersion == "" {
			// Rename via edit is blocked below; use create/delete for that.
		...
		// Mirrors pkg import's enforceImportPolicy.
		if m.auditDB != nil {
...
func rebuildCreateFields(entryType string, prev []formField) []formField {
	...
	case manifest.TypePypi:
		...
		return restoreCursors([]formField{
			...
			{Label: "Name", Value: restore("Name", ""),
				Hint: "pip package specifier, e.g. examplesdk or social-auth-core[openidconnect]"},
			...
			{Label: "Required By", Value: restore("Required By", ""),
				Hint: "comma-separated, e.g. widget,standalone"},
		})
	...
	case manifest.TypeGomod:
		...
		return restoreCursors([]formField{
			...
			{Label: "Name", Value: restore("Name", ""),
				Hint: "module path, e.g. example.com/example-corp/widget-sdk"},
			{Label: "Version", Value: versionVal, Disabled: versionDisabled,
				LabelSelect: true, LabelSelectValue: constraintVal,
				LabelSelectOptions: []string{"exact (=)", "compatible (^)", "patch (~)", "latest (*)"},
				Hint:               "e.g. v1.30.0"},
...
func extractNameFromURL(rawURL, entryType string) string {
	...
	case manifest.TypeGit:
		...
		return ""
	case manifest.TypeGomod:
		// Go module paths are already the name: example.com/example-corp/widget-sdk
		return rawURL
	case manifest.TypeNpm:
