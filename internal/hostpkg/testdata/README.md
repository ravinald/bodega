...
| --- | --- | --- |
| `apt-dpkg-query-ubuntu2604.txt` | `dpkg-query -W -f='${Package}\t${Version}\t${Architecture}\t${Status}\n'` | Ubuntu 26.04 server, arm64 |
| `apt-list-installed-ubuntu2604.txt` | `apt list --installed` | the same host, same moment |
...
| `gomod-go-version-m.txt` | `go version -m $(command -v golangci-lint)` | macOS, go1.27.0 |
...
The 26.04 capture is the scale and the correctness case in one. `dpkg-query -W`
returns 731 rows there; only 728 are `install ok installed`. The other 3 are
`deinstall ok config-files` superseded kernel images, and a parser that skips
the status filter mirrors packages the host does not have. The matching
`apt list --installed` capture lists exactly those 728, so the two formats
cross-check each other and neither is graded against its own parser.

That gap is 3 rows because the capture came from a guest rebuilt often enough
that little accumulates. A long-lived server carries far more, so the status
filter is asserted here rather than stressed. A capture with a wider gap would
be a better fixture; this one still fails if the filter is dropped.
