...
//
// The name is canonicalized here rather than at the import that reads this
// output: `pip list --format=json` reports a distribution under its published
// capitalization, and a catalog written from one holds entries the client that
// inventory came from cannot reach. Every other type is returned unchanged.
func pkg(typ, name, version, url, mode string) manifest.PackageManifest {
	name = manifest.CanonicalName(typ, name)
...
// mergeSameName folds manifests that canonicalize to one name into one, keeping
// every version each carried.
//
// `pip list` reports one row per installed distribution, so the fold is empty on
// a venv. An inventory concatenated from two environments is the case it exists
// for: two manifests for one name reach `pkg import` as two writes, the second
// overwriting the first, and the versions on the loser vanish with no error.
func mergeSameName(pms []manifest.PackageManifest) []manifest.PackageManifest {
	at := make(map[string]int, len(pms))
	out := make([]manifest.PackageManifest, 0, len(pms))
	for _, pm := range pms {
		key := pm.Type + "/" + pm.Name
		i, seen := at[key]
		if !seen {
		...
		for _, ve := range pm.Versions {
			if !slices.ContainsFunc(out[i].Versions, func(have manifest.VersionEntry) bool {
				return have.Version == ve.Version
			}) {
				out[i].Versions = append(out[i].Versions, ve)
			}
		}
	}
	return out
