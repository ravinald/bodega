...
func TestGitRefFromFile(t *testing.T) {
	for _, tc := range []struct {
		...
		{"widget", "widget-v4.5.5.bundle", "v4.5.5", false, true},
		{"widget", "widget-v4.5.5.tar.gz", "v4.5.5", true, true},
		// A ref may carry the separator the filename uses; only the first
		// occurrence delimits the name.
		{"widget", "widget-feature-x.bundle", "feature-x", false, true},
		// Someone else's artifact under this package's path.
		{"widget", "other-v4.5.5.bundle", "", false, false},
		// The suffix is present and the ref is not, which addresses nothing.
		{"widget", "widget-.bundle", "", false, false},
		{"widget", "widget-.tar.gz", "", false, false},
		...
		{"widget", "widget-v4.5.5.zip", "", false, false},
		{"widget", "widget-v4.5.5", "", false, false},
		{"widget", "widget", "", false, false},
		{"widget", "", "", false, false},
	} {
...
func newGitBundleServer(t *testing.T) *httptest.Server {
	...
	if err := store.AddVersion(t.Context(), manifest.TypeGit, "widget", manifest.VersionEntry{
	...
	if err := mem.Put(t.Context(), manifest.GitKey("widget", "v4.5.5", false), []byte("fake-bundle")); err != nil {
		t.Fatalf("Put: %v", err)
	}
...
func TestGitBundleUnparseableFilenameIs404(t *testing.T) {
	...
	if got := gitGetStatus(t, ts.URL+"/git/widget/widget-v4.5.5.bundle"); got != http.StatusOK {
		t.Fatalf("GET the uploaded bundle = %d, want 200", got)
	}

	for _, file := range []string{
		"widget-v4.5.5.zip", // neither suffix
		"widget-.bundle",    // empty ref
		"other-v4.5.5.bundle",
		"widget-v4.5.5",
	} {
		if got := gitGetStatus(t, ts.URL+"/git/widget/"+file); got != http.StatusNotFound {
			t.Errorf("GET /git/widget/%s = %d, want 404", file, got)
		}
	}
}
