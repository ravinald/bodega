...
func TestCIDRProfileEnforcesOnGit(t *testing.T) {
	refusedByCIDRProfile(t, cidrLockedServer(t), "/git/netbox/netbox-v4.5.5.bundle")
}

func TestCIDRProfileEnforcesOnBinary(t *testing.T) {
	refusedByCIDRProfile(t, cidrLockedServer(t), "/binaries/awscli-v2/2.0.0/awscli.zip")
}
...
func TestARequestFromABoundAddressIsNamedInTheAuditRow(t *testing.T) {
	...
	f := bindProfileByCIDR(t, s, "reader", "probe-host", cidrLoopback,
		...
		[]audit.ProfileEntry{{Type: manifest.TypeBinary, Name: "awscli-v2"}})

	if status, body := f.get(t, "/binaries/awscli-v2/2.0.0/awscli.zip"); status != http.StatusOK {
