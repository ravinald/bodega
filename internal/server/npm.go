...
func (s *Server) handleNpm(w http.ResponseWriter, r *http.Request) {
	...
	if idx := strings.Index(fullPath, "/-/"); idx >= 0 {
		pkgName := fullPath[:idx]   // canonical, e.g. "@example-corp/widget-cli"
		tarball := fullPath[idx+3:] // URL form, e.g. "cli-1.4.2.tgz"
...
// @example-corp/widget-cli + widget-cli-1.5.0.tgz gives 1.5.0. "" on unexpected shape.
