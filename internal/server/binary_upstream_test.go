...
const (
	vendorURL = "https://downloads.example.invalid/"
	toolRest  = "widget-tool/1.7.5/widget-tool_1.7.5_linux_amd64.zip"
	toolPkg   = "example-vendor/" + toolRest
	toolKey   = manifest.BinaryPrefix + toolPkg
	toolURL   = vendorURL + toolRest
)
...
func binaryServer(t *testing.T, mode string) *Server {
	...
	s.cfg.BinaryUpstreams = map[string]config.BinaryUpstream{
		"example-vendor": {URL: vendorURL, Mode: config.UpstreamModeOpen},
...
func TestBinaryOpenServesFromCache(t *testing.T) {
	s := binaryServer(t, "observe")
	if err := s.typeStore(manifest.TypeBinary).Put(t.Context(), toolKey, []byte("zip")); err != nil {
		t.Fatalf("seed cache: %v", err)
	}

	if got := getBinary(t, s, "/binaries/"+toolPkg); got != http.StatusOK {
		t.Errorf("GET cached open-mode binary = %d, want 200", got)
	}
	rows := waitForBinaryRows(t, s, audit.DecisionNoPolicy, 1)
	if want := "https://downloads.example.invalid/widget-tool/1.7.5/widget-tool_1.7.5_linux_amd64.zip"; rows[0].UpstreamURL != want {
...
func TestBinaryOpenMatrix(t *testing.T) {
	for _, tc := range []struct {
		...
		{"observe, allow", "observe", "https://downloads.example.invalid/widget-tool/", http.StatusBadGateway, audit.DecisionAllowed},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := getBinary(t, s, "/binaries/"+toolPkg); got != tc.wantStatus {
			...
			if rows[0].PkgName != toolPkg {
				t.Errorf("pkg_name = %q, want %q — the promote target is <namespace>/<rest>", rows[0].PkgName, toolPkg)
			}
			if rows[0].UpstreamURL != toolURL {
				t.Errorf("upstream_url = %q, want %q", rows[0].UpstreamURL, toolURL)
			}
			...
			if want := "https://downloads.example.invalid/widget-tool/"; rows[0].PatternHint != want {
...
func TestUpstreamViolationRefusedAtEveryDiscoverMode(t *testing.T) {
	for _, mode := range []string{"", "observe"} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if got := getBinary(t, s, "/binaries/"+toolPkg); got != http.StatusForbidden {
...
// observable before any rule exists, and once the widget-tool prefix is allowed
// the denial has to come from a package outside it: other-tool, on the same open
// namespace. decision is part of the discovery upsert key, so the two
// widget-tool requests leave two rows rather than overwriting each other.
func TestObserveRecordsEveryDecisionAndStillEnforces(t *testing.T) {
	const otherToolPkg = "example-vendor/other-tool/1.16.0/other-tool_1.16.0_linux_amd64.zip"
	s := binaryServer(t, "observe")
	...
	for _, tc := range []struct {
		...
		{"no_policy", "", "/binaries/" + toolPkg, http.StatusBadGateway, audit.DecisionNoPolicy},
		{"allowed", vendorURL + "widget-tool/", "/binaries/" + toolPkg, http.StatusBadGateway, audit.DecisionAllowed},
		// The 403 is the assertion, not the row: under learn mode this same
		// request returned 502 because the fetch went ahead anyway.
		{"denied", "", "/binaries/" + otherToolPkg, http.StatusForbidden, audit.DecisionDenied},
