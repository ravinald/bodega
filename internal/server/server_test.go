...
func newTestServer(t *testing.T) (*httptest.Server, *storage.Memory) {
	...
	_ = store.AddVersion(ctx, manifest.TypeGit, "widget", manifest.VersionEntry{
		URL: "https://example.com/example-corp/widget",
		Ref: "v4.5.5",
	})
	_ = store.AddVersion(ctx, manifest.TypePypi, "examplesdk", manifest.VersionEntry{})
	_ = store.AddVersion(ctx, manifest.TypePypi, "django", manifest.VersionEntry{})
	_ = store.AddVersion(ctx, manifest.TypeBinary, "example-tool-v2", manifest.VersionEntry{
		Version: "2.0.0",
	...
	mock := memStore(map[string]string{
		...
		"pypi/wheels/examplesdk-1.35.0-py3-none-any.whl":                             "fake-wheel-examplesdk",
		"pypi/wheels/django-5.0.0-py3-none-any.whl":                                  "fake-wheel-django",
		"repos/widget/widget-v4.5.5.bundle":                                          "fake-bundle",
		"binaries/example-tool-v2/2.0.0/example-tool.zip":                            "fake-binary",
	})
...
func TestPypiRootIndex(t *testing.T) {
	...
	body, _ := io.ReadAll(resp.Body)
	html := string(body)
	for _, pkg := range []string{"examplesdk", "django"} {
...
func TestPypiPackageIndex(t *testing.T) {
	ts, _ := newTestServer(t)
	resp, err := http.Get(ts.URL + "/pypi/simple/examplesdk/")
	if err != nil {
		t.Fatalf("GET /pypi/simple/examplesdk/: %v", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Errorf("status = %d, want 200", resp.StatusCode)
	...
	body, _ := io.ReadAll(resp.Body)
	html := string(body)
	if !strings.Contains(html, "examplesdk-1.35.0-py3-none-any.whl") {
		t.Errorf("package index missing wheel link: %s", html)
	}
	if strings.Contains(html, "django") {
		t.Error("examplesdk index should not contain django wheels")
	}
}

func TestPypiPackageIndexNormalization(t *testing.T) {
	// pip normalizes package names, so ExampleSDK and examplesdk name the same
	// package.
	ts, _ := newTestServer(t)
	resp, err := http.Get(ts.URL + "/pypi/simple/ExampleSDK/")
	if err != nil {
...
func TestPypiWheelProxy(t *testing.T) {
	ts, _ := newTestServer(t)
	resp, err := http.Get(ts.URL + "/pypi/wheels/examplesdk-1.35.0-py3-none-any.whl")
	if err != nil {
		t.Fatalf("request failed: %v", err)
	}
	defer resp.Body.Close()
	...
	body, _ := io.ReadAll(resp.Body)
	if string(body) != "fake-wheel-examplesdk" {
		t.Errorf("body = %q, want fake-wheel-examplesdk", string(body))
	}
...
func TestGitBundleProxy(t *testing.T) {
	ts, _ := newTestServer(t)
	resp, err := http.Get(ts.URL + "/git/widget/widget-v4.5.5.bundle")
	if err != nil {
		t.Fatalf("request failed: %v", err)
	}
	defer resp.Body.Close()
...
func TestBinaryProxy(t *testing.T) {
	ts, _ := newTestServer(t)
	resp, err := http.Get(ts.URL + "/binaries/example-tool-v2/2.0.0/example-tool.zip")
	if err != nil {
		t.Fatalf("request failed: %v", err)
	}
	defer resp.Body.Close()
...
func TestAPIPackageSingle(t *testing.T) {
	...
	tests := []struct {
		...
		{"/api/v1/packages/git/widget", http.StatusOK, "widget"},
		{"/api/v1/packages/binary/example-tool-v2", http.StatusOK, "example-tool-v2"},
	}
...
func TestAPIPackageVersion(t *testing.T) {
	...
	tests := []struct {
		...
		{"known version by Ref field", "/api/v1/packages/git/widget/v4.5.5", http.StatusOK, ""},
...
func TestServeOverLocalBackend(t *testing.T) {
	...
	if !strings.Contains(index, ">examplesdk<") {
		t.Errorf("pypi index missing examplesdk:\n%s", index)
	}
}
