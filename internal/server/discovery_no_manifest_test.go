...
func TestUnknownPackageRecordsNoManifest(t *testing.T) {
	...
	for _, path := range []string{
		"/go/example.com/example-corp/widget-sdk/@v/v1.30.0.info",
		"/pypi/wheels/examplesdk-1.26.0-py3-none-any.whl",
	...
	for _, want := range []struct {
		...
		{manifest.TypeGomod, "example.com/example-corp/widget-sdk", "v1.30.0", "https://proxy.golang.org/example.com/example-corp/widget-sdk/@v/v1.30.0.info"},
		...
		{manifest.TypePypi, "examplesdk", "1.26.0", "https://pypi.org/simple/examplesdk/"},
...
// B75 R4: pip meets an uncataloged distribution at the simple index and never
// composes a wheel URL, so the wheel route's recorder saw nothing and the
// documented bootstrap — observe, install, generate-manifests — returned an
// empty catalog on an empty store. The install that found this drove fifty-one
// rounds of a shell loop over pip's stderr instead.
func TestUncatalogedPypiIndexReadRecordsNoManifest(t *testing.T) {
	s := newDiscoveryServer(t)
	ts := httptest.NewServer(s.Handler())
	t.Cleanup(ts.Close)

	resp, err := http.Get(ts.URL + "/pypi/simple/this-dist-does-not-exist-here/") //nolint:gosec,noctx // test-owned loopback URL
	if err != nil {
		t.Fatalf("GET: %v", err)
	}
	_ = resp.Body.Close()
	if resp.StatusCode != http.StatusNotFound {
		t.Errorf("status = %d, want 404 — this records the miss, it does not serve it", resp.StatusCode)
	}

	rows := waitForDiscovery(t, s, 1)
	row := rows[0]
	if row.RegistryType != manifest.TypePypi || row.PkgName != "this-dist-does-not-exist-here" {
		t.Fatalf("row = %+v, want the pypi distribution the index named", row)
	}
	if row.PkgVersion != "" {
		t.Errorf("pkg_version = %q; a simple-index read names no version", row.PkgVersion)
	}
	// The URL a promote has to be able to fetch: a wheel URL is read out of this
	// document rather than composed, so it is the only one this branch knows.
	if want := "https://pypi.org/simple/this-dist-does-not-exist-here/"; row.UpstreamURL != want {
		t.Errorf("upstream_url = %q, want %q", row.UpstreamURL, want)
	}
}

// A distribution the store can answer for is not a miss, whichever route
// answers it. Recording one would tell an operator to catalog what is
// cataloged.
func TestCatalogedPypiIndexReadRecordsNothing(t *testing.T) {
	s := newDiscoveryServer(t)
	if err := s.store.AddVersion(t.Context(), manifest.TypePypi, "six", manifest.VersionEntry{
		Version: "1.16.0",
	}); err != nil {
		t.Fatalf("seed manifest: %v", err)
	}
	...
	resp, err := http.Get(ts.URL + "/pypi/simple/six/") //nolint:gosec,noctx // test-owned loopback URL
	if err != nil {
		t.Fatalf("GET: %v", err)
	}
	_ = resp.Body.Close()
...
// A manifest can be absent while the cache still holds wheels: `pkg delete
// pypi` leaves the bytes in storage on purpose. The index then answers 200 from
// the cache, so guarding the recorder on an empty listing made the observation
// depend on what the cache happened to hold rather than on what the catalog
// does not.
func TestUncatalogedPypiIndexReadWithCachedWheelsStillRecords(t *testing.T) {
	s := newDiscoveryServer(t)
	seed(t, s, manifest.TypePypi, map[string]string{
		manifest.PypiWheelPrefix + "six-1.16.0-py3-none-any.whl": "wheel bytes",
	})
	ts := httptest.NewServer(s.Handler())
	t.Cleanup(ts.Close)
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("status = %d, want 200 — the cached wheel is still served", resp.StatusCode)
	}

	rows := waitForDiscovery(t, s, 1)
	if rows[0].PkgName != "six" || rows[0].RegistryType != manifest.TypePypi {
		t.Errorf("row = %+v, want the pypi distribution the index named", rows[0])
	}
	if want := "https://pypi.org/simple/six/"; rows[0].UpstreamURL != want {
		t.Errorf("upstream_url = %q, want %q", rows[0].UpstreamURL, want)
	}
}
