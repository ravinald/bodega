...
const (
	...
	// mirrored stanza names by default. A mirrored codename serves whatever
	...
	// there and valid.
	MirroredNote = `This codename is mirrored from upstream, signature included, so apt verifies it against the distro keyring already on the host. Do not add [trusted=yes] or Signed-By: here — bodega does not sign a mirrored codename, and either one would replace a working check with a worse one.`
