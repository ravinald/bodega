...
const (
	...
	// Component is the only component bodega generates, and the whole
	// Components: line for a suite bodega builds and signs.
	Component = "main"

	// MirroredComponents is the Components: line for a mirrored codename whose
	// apt_upstreams entry names none. A mirrored codename serves whatever its
	// upstream publishes, bodega parses no index, and reading the upstream
	// Release to find out would make startup depend on a host bodega only
	// proxies.
	//
	// Ubuntu's four rather than "main", because the two failures are not
	// symmetric. A component the upstream does not publish fails at "apt
	// update", loudly, naming the component and the URL that 404d. A component
	// the line leaves out fails much later at "apt-get install", after update
	// reported success, as a package apt cannot locate — which is the same
	// terminal line an architecture mismatch produces. Debian publishes "main
	// contrib non-free non-free-firmware" and needs the key set.
	MirroredComponents = "main restricted universe multiverse"
	...
	// MirroredNote names what verifies a mirrored codename. bodega proxies the
	...
	// there and valid.
	MirroredNote = `This codename is mirrored from upstream, signature included, so apt verifies it against the distro keyring already on the host. Do not add [trusted=yes] or Signed-By: here — bodega does not sign a mirrored codename, and either one would replace a working check with a worse one. Check the Components: line against what the upstream publishes and set "components" on the apt_upstreams entry if it differs: a component missing from that line 404s at the pool on "apt-get install", after "apt update" reported success.`
...
type State struct {
	...
	// Components go on the Components: line for a mirrored codename. Empty
	// renders MirroredComponents. It is read only when Mirrored is set: a suite
	// bodega generates carries exactly the one component bodega builds.
	Components []string

	// Mirrored marks these codenames as proxied from an upstream archive
	// rather than generated by bodega, which changes what authenticates them
	// and therefore the whole trust half of the stanza. Signed is not consulted
...
type Sources struct {
	...
	// Components is the rendered Components: line, space-separated, so a
	// consumer reading the JSON does not have to parse it back out of Deb822.
...
// MirroredComponentList is what a mirrored codename's Components: line names:
// the components its upstreams declare, or MirroredComponents when they declare
// none.
//
// Exported because a caller that groups codenames into stanzas has to group on
// the same answer Render will print. Grouping on the configured value alone put
// a codename with no components and one naming Ubuntu's four in two stanzas
// carrying an identical Components: line, which is two sources entries where
// apt wants one.
func MirroredComponentList(named []string) []string {
	...
	if len(out) == 0 {
		return strings.Fields(MirroredComponents)
	}
	return out
}
...
func Render(st State) Sources {
	...
	if st.Mirrored {
		components = strings.Join(MirroredComponentList(st.Components), " ")
	}
	...
	out := Sources{Signed: st.Signed, Suite: suites[0], URI: uri, Mirrored: st.Mirrored, Components: components}
	switch {
	case st.Mirrored:
		...
		out.Signed = true
		out.OneLine = fmt.Sprintf("deb %s %s %s", uri, suites[0], components)
