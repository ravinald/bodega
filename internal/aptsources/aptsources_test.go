...
func TestMirroredSuiteCarriesNeitherTrustOption(t *testing.T) {
	for _, signed := range []bool{true, false} {
		...
		if want := "deb https://bodega.example.com/apt/ noble " + MirroredComponents; got.OneLine != want {
			t.Errorf("one-line = %q, want %q", got.OneLine, want)
		}
	}
}
...
// B75 R6: the banner printed "Components: main" for a codename serving whatever
// its upstream publishes, and docs/usage.md printed the four-component line for
// the same configuration. Copying the banner is what the document tells the
// reader to do, and "apt-get install redis-server" then 404s at the pool after
// apt update reported success, because redis is in universe.
func TestMirroredStanzaNamesTheComponentsTheCodenameServes(t *testing.T) {
	base := State{PublicURL: "https://bodega.example.com", Suites: []string{"noble"}}

	mirrored := base
	mirrored.Mirrored = true
	got := Render(mirrored)
	if !strings.Contains(got.Deb822, "Components: "+MirroredComponents) {
		t.Errorf("a mirrored codename with no components configured renders:\n%s", got.Deb822)
	}
	if got.Components != MirroredComponents {
		t.Errorf("Components = %q, want %q", got.Components, MirroredComponents)
	}

	debian := mirrored
	debian.Components = []string{"main", "contrib", "non-free", "non-free-firmware"}
	got = Render(debian)
	if !strings.Contains(got.Deb822, "Components: main contrib non-free non-free-firmware") {
		t.Errorf("a configured component set did not reach the stanza:\n%s", got.Deb822)
	}
	if !strings.HasSuffix(got.OneLine, "noble main contrib non-free non-free-firmware") {
		t.Errorf("one-line = %q, want the configured components", got.OneLine)
	}

	// A generated suite carries the one component bodega builds, whatever is
	// set: bodega signs what it generates and generates only main.
	generated := base
	generated.Components = []string{"universe"}
	got = Render(generated)
	if !strings.Contains(got.Deb822, "Components: "+Component) {
		t.Errorf("a generated suite took a component bodega does not publish:\n%s", got.Deb822)
	}
}
