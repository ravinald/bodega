...
//
// The rule itself is manifest.CanonicalName, which is also what the store
// composes a path from: a gate that normalized on its own terms would permit a
// name no read resolves.
func Key(typ, name string) string {
	return manifest.CanonicalName(typ, name)
}
