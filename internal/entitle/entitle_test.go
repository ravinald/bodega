...
// B75 R1: the gate and the store answer to one canonicalizer. entitle held its
// own copy, so a profile could permit a name no read resolves — and the
// difference only showed on a distribution spelled two ways.
func TestKeyCanonicalizesEverySpellingOfOneDistribution(t *testing.T) {
	...
	for in, want := range cases {
		if got := Key(manifest.TypePypi, in); got != want {
			t.Errorf("Key(pypi, %q) = %q, want %q", in, got, want)
		}
		if got := Key(manifest.TypePypi, in); got != manifest.CanonicalName(manifest.TypePypi, in) {
			t.Errorf("Key(pypi, %q) = %q, and the store composes %q", in, got, manifest.CanonicalName(manifest.TypePypi, in))
		}
	}
	// Every other type is identity: an npm scope is case-sensitive and a Go
	// module path is not a distribution name.
	for _, c := range []struct{ typ, name string }{
		...
		if got := Key(c.typ, c.name); got != c.name {
			t.Errorf("Key(%s, %q) = %q, want it unchanged", c.typ, c.name, got)
		}
	}
}
