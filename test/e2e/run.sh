...
# dev guests described in test/e2e/README.md.
#
...
# `userdel bodega`. A production host commonly differs from a dev guest by one
# label, so the allowlist is declared separately from the host map and checked
# against it: a typo in either one trips this. Both come from hosts.env, and
# neither is taken from a flag, because a flag would make the destructive
# suites one typo from something that matters.
