...
- `ssh` from the workstation as a user with passwordless `sudo`, by key. The
  harness never prompts, so a password-gated `sudo` hangs the run.
- Outbound network to whatever upstream registries the suites fetch from.
- The server guest reachable from the client guest on port 8080, by the name
  `E2E_SERVER_HOST` resolves to. Suite 10 writes `/etc/hosts` entries when DNS
  does not answer.
...
not write is already there. Every `make` it runs on `freebsd` runs as
`bodega-e2e-fetch`, an account with no `sudo` grant that the suite creates and
removes, because a `make` that can elevate can lift the mounts. It also moves
the server's `binaries/distfiles` storage, where suite 47 caches, aside under
the storage root for the length of the suite and puts it back at the end.
Suite 48 ships an `internal/storage` test binary to `freebsd-server`,
...
- A `| tail -5` made the pipeline report `tail`'s exit code, so npm 404ing and
  ...
  guest.
- The admin-gated read checks ran _after_ the admin list was widened, so they
...
- **TLS.** This harness runs against a plaintext listener, which is why the pip
  checks pass `--trusted-host`. The unit on the guests has only ever run with
  `allow_plaintext`. The base URL is a variable, so turning it on is a config
...
- **A scripted guest reset.** Restoring is a manual clone-and-rename in the
  hypervisor, which
