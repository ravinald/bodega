...
#
# This suite mutates freebsd-client: it installs a bodega binary, appends to
...
E2E_HOST=freebsd-client
if e2e_on freebsd-client true 2>/dev/null; then
	e2e_record FBSD-00 PASS "freebsd-client answers ssh" "reachable" "reachable" "ssh $E2E_FREEBSD_CLIENT_HOST true" 0 ""
else
	e2e_record FBSD-00 FAIL "freebsd-client answers ssh" "reachable" "$E2E_ERR" "ssh $E2E_FREEBSD_CLIENT_HOST true" "$E2E_RC" "test/e2e/README.md"
	e2e_block FBSD-BLOCK-HOST "the FreeBSD client" "freebsd-client is unreachable; power it on"
	return 0
fi
...
if [ "${E2E_SHIP:-yes}" = yes ]; then
	...
	E2E_HOST=freebsd-client
	if e2e_put freebsd-client "$FBSD_BIN" /tmp/bodega-under-test &&
		e2e_on freebsd-client "sudo install -o root -g wheel -m 0755 /tmp/bodega-under-test /usr/local/bin/bodega"; then
		e2e_record FBSD-02 PASS "freebsd-client takes the new binary" "installed" "installed" "install /usr/local/bin/bodega" 0 ""
	else
		e2e_record FBSD-02 FAIL "freebsd-client takes the new binary" "installed" "$E2E_ERR" "install /usr/local/bin/bodega" "$E2E_RC" ""
	fi
else
	e2e_skip FBSD-01 "make cross builds the freebsd/arm64 binary" "--no-ship"
	e2e_skip FBSD-02 "freebsd-client takes the new binary" "--no-ship"
fi

E2E_HOST=freebsd-client
e2e_on freebsd-client "bodega --version" || true
check_contains FBSD-03 "freebsd-client runs the commit under test" \
...
e2e_on freebsd-client "sudo sed -i '' '/# bodega-e2e\$/d' /etc/hosts && \
	printf '%s %s # bodega-e2e\n' '$E2E_SERVER_ADDR' '$E2E_SERVER_HOST' | sudo tee -a /etc/hosts >/dev/null" || true
e2e_on freebsd-client "curl -sS -o /dev/null -w '%{http_code}' --max-time 10 '$E2E_BASE_URL/healthz'" || true
check_eq FBSD-04 "freebsd-client reaches the server by name" "200" "$E2E_OUT" \
...
e2e_on freebsd-client "pkg config abi" || true
fbsd_abi="$E2E_OUT"
check_matches FBSD-PKG-00 "freebsd-client reports its pkg ABI" '^FreeBSD:[0-9]+:[a-z0-9]+$' \
...
E2E_HOST=freebsd-client
e2e_on freebsd-client "sudo pkg delete -y tree >/dev/null 2>&1; true" || true
e2e_on freebsd-client "sudo bodega doctor --write-pkg-repo --url '$E2E_BASE_URL' --allow-plaintext" || true
...
e2e_on freebsd-client "pkg -vv | awk '/^  FreeBSD-ports:/{r=1} r && /enabled/{v=\$NF; gsub(/[^a-z]/, \"\", v); print v; exit}'" || true
...
e2e_on freebsd-client "sudo pkg update -f 2>&1 | tail -3" || true
...
e2e_on freebsd-client "sudo pkg install -y -r bodega-e2e-latest tree 2>&1 | tail -3" || true
...
e2e_on freebsd-client "pkg query '%n %R' tree" || true
...
e2e_on freebsd-client "sudo pkg delete -y tree >/dev/null && sudo rm -f /usr/local/etc/pkg/repos/bodega.conf && \
...
E2E_HOST=freebsd-client
e2e_on freebsd-client "rm -rf $FBSD_ROOT && mkdir -p $FBSD_ROOT/distfiles $FBSD_ROOT/wrk && \
...
check_eq FBSD-PORTS-01 "freebsd-client receives the ports tree the release MANIFEST names" \
...
e2e_on freebsd-client "tar -xf $FBSD_ROOT/ports.txz -C $FBSD_ROOT \
...
E2E_HOST=freebsd-client
e2e_on freebsd-client "$fbsd_make MASTER_SITE_OVERRIDE='$E2E_BASE_URL/binaries/distfiles/\${DIST_SUBDIR}/' fetch 2>&1 | \
...
e2e_on freebsd-client "$fbsd_make checksum 2>&1 | tail -1" || true
...
E2E_HOST=freebsd-client
e2e_on freebsd-client "rm -rf $FBSD_ROOT" || true
