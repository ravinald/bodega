...
# The two formats of one command, compared from one run. CSV emitted pkg_name
# and JSON emitted PkgName, so a consumer written against the column names in
# docs/usage.md read nothing out of either format. Set
# equality rather than order: JSON object keys carry none, and asserting a
# sequence would fail on a reordering that breaks no consumer.
e2e_bodega server "discover export csv pypi" || true
csv_header="$(printf '%s' "$E2E_OUT" | head -1 | tr ',' '\n' | sort | tr '\n' ' ')"
e2e_bodega server "discover export json pypi" || true
json_keys="$(printf '%s' "$E2E_OUT" | jq -r 'if length > 0 then .[0] | keys[] else empty end' 2>/dev/null | sort | tr '\n' ' ')"
if [ -z "${json_keys// /}" ]; then
	e2e_skip JSON-KEYS-01 "discover export csv and json name the same fields" \
		"this guest's pypi discovery table is empty, so there is no object to read keys off" "cmd/bodega/cmd_discover.go:377"
else
	check_eq JSON-KEYS-01 "discover export csv and json name the same fields" \
		"$csv_header" "$json_keys" \
		"internal/audit/discovery.go:54; the CSV header at cmd_discover.go:377 against the keys of the first JSON object from the same table" \
		"bodega discover export csv pypi | head -1, against discover export json pypi | jq '.[0] | keys'"
fi
unset csv_header json_keys
