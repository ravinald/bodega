...
if e2e_on server true 2>/dev/null; then
	...
	e2e_record PRE-01 FAIL "server answers ssh" "reachable" "$E2E_ERR" "ssh $E2E_SERVER_HOST true" "$E2E_RC" "docs-internal/DEV_HOSTS.md"
fi
...
if e2e_on client true 2>/dev/null; then
	...
	e2e_record PRE-02 FAIL "client answers ssh" "reachable" "$E2E_ERR" "ssh $E2E_CLIENT_HOST true" "$E2E_RC" "docs-internal/DEV_HOSTS.md"
fi

if [ "$E2E_SERVER_UP" != yes ] || [ "$E2E_CLIENT_UP" != yes ]; then
	E2E_HOST=local
	e2e_block PRE-BLOCK "remaining preflight checks" "a guest is unreachable; power it on in UTM" "docs-internal/DEV_HOSTS.md"
	return 0
fi
...
check_eq PRE-03 "server is the host it claims to be" \
	"${E2E_SERVER_HOST%%.*}" "$E2E_OUT" "docs-internal/DEV_HOSTS.md" "hostname -s"

E2E_HOST=client
e2e_on client "hostname -s" || true
check_eq PRE-04 "client is the host it claims to be" \
	"${E2E_CLIENT_HOST%%.*}" "$E2E_OUT" "docs-internal/DEV_HOSTS.md" "hostname -s"
...
for h in server client; do
	...
	check_eq "PRE-0$([ "$h" = server ] && echo 5 || echo 6)" \
		"$h sudo is passwordless" "0" "$E2E_OUT" "docs-internal/DEV_HOSTS.md" "sudo -n id -u" "$E2E_RC"
done
...
for pair in "server:$E2E_SERVER_HOST" "client:$E2E_CLIENT_HOST"; do
	...
	check_matches "PRE-10-$alias" "this workstation resolves $name" \
		...
		"docs-internal/DEV_HOSTS.md" "dscacheutil -q host -a name $name"
