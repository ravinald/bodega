...
Clients can use the server as follows:
  ...
  apt:  deb [trusted=yes] http://reman-host:8080/apt/ noble main
  pip:  pip install --index-url http://reman-host:8080/pypi/simple/ <package>
  git:  curl http://reman-host:8080/git/<name>/<name>-<ref>.bundle -o <name>.bundle
...
TLS can be enabled in two ways:
  ...
  --tls-autocert --tls-domain  Automatic Let's Encrypt certificates`,
	...
	cmd.Flags().StringVar(&tlsDomain, "tls-domain", "", "Domain name for autocert (e.g. reman.example.com)")
	return cmd
}
