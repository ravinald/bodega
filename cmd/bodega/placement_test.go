...
func TestPlacementIsRecordedBeforeTheUpload(t *testing.T) {
	...
	addBinary(t, store, "example-tool", "2.0.0", "")

	st, err := pl.ForVersion(t.Context(), manifest.TypeBinary, "example-tool", "2.0.0", "binaries/example-tool/2.0.0/example-tool.zip")
	if err != nil {
		t.Fatalf("forVersion: %v", err)
	}
	if got := recordedStorage(t, store, manifest.TypeBinary, "example-tool", "2.0.0"); got != "bulk" {
...
func TestDefaultPlacementStaysTheZeroValue(t *testing.T) {
	...
	if _, err := pl.ForVersion(t.Context(), manifest.TypeBinary, "example-tool", "2.0.0", "binaries/example-tool/2.0.0/example-tool.zip"); err != nil {
		t.Fatalf("forVersion: %v", err)
	}
	if got := recordedStorage(t, store, manifest.TypeBinary, "example-tool", "2.0.0"); got != "" {
...
func TestReUploadHonorsTheRecordedName(t *testing.T) {
	...
	addBinary(t, store, "example-tool", "2.0.0", "bulk")
...
func TestValidateManifestRejectsAnUnknownStoragePolicy(t *testing.T) {
	...
	pm := &manifest.PackageManifest{Name: "widget", Type: manifest.TypeGit, StoragePolicy: "archive"}
...
func TestGitPlacementSurvivesTheHandoffToTheServer(t *testing.T) {
	...
	store := manifest.NewLocalStore(t.TempDir())
	if err := store.AddVersion(ctx, manifest.TypeGit, "widget", manifest.VersionEntry{
		Ref: "v4.5.5",
		URL: "https://example.com/example-corp/widget",
	}); err != nil {
		t.Fatalf("AddVersion: %v", err)
	}
	key := manifest.GitKey("widget", "v4.5.5", false)
	...
	if got := recordedStorage(t, store, manifest.TypeGit, "widget", "v4.5.5"); got != "bulk" {
	...
	assertServes(t, ts.URL+"/git/widget/widget-v4.5.5.bundle", "from-bulk")
}
