...
func groupFixture(t *testing.T) (*config.Config, *manifest.Store) {
	...
	seed := []struct {
		...
		{manifest.TypeBinary, "awscli-v2", []string{"mirror-set"}, ""},
		{manifest.TypeBinary, "kubectl", []string{"mirror-set"}, "cold"},
		{manifest.TypePypi, "boto3", []string{"mirror-set"}, ""},
...
func TestGroupMembersSeparateMembershipFromPlacement(t *testing.T) {
	...
	if m := got["binary/awscli-v2"]; !m.Deciding {
		t.Errorf("binary/awscli-v2 deciding = false (%s), want the group to place it", m.Why)
	}
	...
	if m := got["pypi/boto3"]; m.Deciding || !strings.Contains(m.Why, "pypi") {
		t.Errorf("pypi/boto3 = %+v, want the group reported as not consulted", m)
	}
