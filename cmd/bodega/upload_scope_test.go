...
func TestParseUploadArgsSplitsTypesFromTheSelector(t *testing.T) {
	types, selector, err := parseUploadArgs([]string{manifest.TypeBinary, "awscli-v2@2.15.0"}, scopeStore(t))
	if err != nil {
...
func TestStorageNeedsOneVersion(t *testing.T) {
	...
	err := applyUploadScope(cfg, scopePlacer(t, cfg, store), []string{manifest.TypeBinary}, "awscli-v2", "bulk")
	if err == nil {
...
func TestStorageNeedsOneType(t *testing.T) {
	cfg, store := scopeConfig(t), scopeStore(t)
	err := applyUploadScope(cfg, scopePlacer(t, cfg, store), manifest.AllTypes, "awscli-v2@2.15.0", "bulk")
	if err == nil {
...
func TestStorageRefusesADirectoryPlacedType(t *testing.T) {
	cfg, store := scopeConfig(t), scopeStore(t)
	err := applyUploadScope(cfg, scopePlacer(t, cfg, store), []string{manifest.TypePypi}, "boto3@1.26.0", "bulk")
	if err == nil {
...
func TestStorageRefusesAnUnconfiguredBackend(t *testing.T) {
	cfg, store := scopeConfig(t), scopeStore(t)
	err := applyUploadScope(cfg, scopePlacer(t, cfg, store), []string{manifest.TypeBinary}, "awscli-v2@2.15.0", "nowhere")
	if err == nil {
...
func TestConfirmPlacedFailsARunThatNeverReachedTheVersion(t *testing.T) {
	...
	if err := applyUploadScope(cfg, pl, []string{manifest.TypeBinary}, "awscli-v2@2.15.0", "bulk"); err != nil {
		t.Fatalf("applyUploadScope: %v", err)
	}
	err := confirmPlaced(pl, manifest.TypeBinary, "awscli-v2@2.15.0", "bulk")
	if err == nil {
		t.Fatal("confirmPlaced passed a run that placed nothing")
	}
	for _, want := range []string{"awscli-v2", "2.15.0", "bulk"} {
...
func TestPrintDriftNamesTheMoveForEachRow(t *testing.T) {
	...
	printDrift(out, []placement.DriftRow{
		{Type: manifest.TypeBinary, Package: "awscli-v2", Version: "2.15.0", On: "default", Rule: "bulk"},
		{Type: manifest.TypePypi, Package: "boto3", Version: "1.26.0", On: "default", Rule: "archive"},
	})
	got := out.String()
...
func placeFixture(t *testing.T) (defaultPath, bulkPath, manifestDir string) {
	...
	artifact := filepath.Join(buildRoot, "binaries", "awscli-v2", "2.15.0", "awscli.zip")
	if err := os.MkdirAll(filepath.Dir(artifact), 0o755); err != nil {
		t.Fatalf("mkdir binaries: %v", err)
	}
	if err := os.WriteFile(artifact, []byte("awscli"), 0o644); err != nil {
...
func TestSyncStorageWritesTheNamedBackendAndRecordsIt(t *testing.T) {
	...
	cmd.SetArgs([]string{manifest.TypeBinary, "awscli-v2@2.15.0", "--storage", "bulk"})
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
	if err := cmd.Execute(); err != nil {
		t.Fatalf("bodega build sync binary awscli-v2@2.15.0 --storage bulk: %v", err)
	}
	...
	pm, err := manifest.NewLocalStore(manifestDir).GetPackage(t.Context(), manifest.TypeBinary, "awscli-v2")
	if err != nil || pm == nil {
...
func TestSyncStorageFailsWhenTheVersionIsNotThere(t *testing.T) {
	...
	cmd := newSyncCmd(&globalFlags{})
	cmd.SetArgs([]string{manifest.TypeBinary, "awscli-v2@2.15.1", "--storage", "bulk"})
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
	err := cmd.Execute()
	if err == nil {
	...
	if _, statErr := os.Stat(filepath.Join(bulkPath, filepath.FromSlash(
		manifest.BinaryKey("awscli-v2", "2.15.0", "awscli.zip")))); statErr == nil {
