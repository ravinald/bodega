...
func TestWriteEditBuffer_SafeName(t *testing.T) {
	path, err := writeEditBuffer("npm", "@example-corp/widget-cli", "1.5.0", []byte(`{"x":1}`))
	if err != nil {
		t.Fatalf("writeEditBuffer: %v", err)
	}
	defer os.Remove(path)
	...
	if !strings.HasPrefix(base, "bodega-edit-npm-example-corp_widget-cli-1.5.0-") {
...
func relabelFixture(t *testing.T, objectOn ...string) {
	...
	for _, backend := range objectOn {
		...
		if err := os.WriteFile(obj, []byte("example-tool"), 0o644); err != nil {
...
func TestEditRefusesAStorageChangeThatStrandsTheArtifact(t *testing.T) {
	...
	cmd.SetArgs([]string{manifest.TypeBinary, "example-tool-v2", "2.15.0", "--editor", relabelEditor(t, "bulk")})
