...
// already provides and which was not enough: in the failure that produced this
// check the unit reached active (running), /healthz answered 200, and apt
// clients got a syntactically valid Release whose Packages digest was the
// SHA-256 of the empty string. A repository server that publishes an empty
// index over a root that does not exist has told every client the packages
// were withdrawn. Refusing is the same posture bodega already takes on a
// missing TLS pair.
//
