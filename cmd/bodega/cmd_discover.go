...
//
// Every name is canonicalized for its type on the way out. A row written before
// the wheel route canonicalized carries pip's spelling, and promoting it
// verbatim writes the entry under a name the store then rewrites — so the line
// this command printed named a package the operator could not find afterwards.
func buildManifestEntries(rows []audit.DiscoveryRow) (entries []manifestEntry, noURL, noVersion []string) {
	...
	for _, row := range rows {
		...
		name := manifest.CanonicalName(row.RegistryType, row.PkgName)
		if row.UpstreamURL == "" {
			if !seenNoURL[name] {
				seenNoURL[name] = true
				noURL = append(noURL, name)
			}
			continue
		}
		if row.PkgVersion == "" && !versionlessNamesArtifact(row.RegistryType) {
			if !seenNoVersion[name] {
				seenNoVersion[name] = true
		...
		entries = append(entries, manifestEntry{PkgName: name, Entry: ve})
	}
...
// repository or a download URL is complete without a version. pypi composes no
// version either: the field is the index root, the server resolves a wheel by
// reading /simple/<dist>/ and the builder resolves one through pip, so an entry
// naming only the distribution is what a proxy-mode pypi entry looks like. The
// remaining types compose the version into the fetch path (internal/builder
// gomod, npm, helm and cargo all append it), and an open entry there resolves to
// a URL that 404s. gomod produces these constantly: go resolves an unknown
// module through /@v/list and /@v/@latest, and neither request carries a
// version.
func versionlessNamesArtifact(regType string) bool {
	switch regType {
	case manifest.TypeGit, manifest.TypeBinary, manifest.TypePypi:
		return true
	}
	return false
}
...
func buildGeneratedManifests(
	...
	for _, row := range rows {
		...
		// Canonical, so two spellings of one distribution group into one
		// manifest rather than two that the store then writes to the same path,
		// the second overwriting the first.
		k := generateKey{Type: row.RegistryType, Name: manifest.CanonicalName(row.RegistryType, row.PkgName)}
