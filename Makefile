...
# A shell command ci-drift runs last, for a checkout whose merge gate is driven
# by a local tool with its own copy of the job list. Empty here on purpose:
# local.mk is not committed, so a clone gates on CI and `check` alone.
LOCAL_DRIFT_CHECK ?= true
-include local.mk
...
#
# Out of `check` on purpose: it needs both guests powered on, installs
# ecosystem clients, and ends in a destructive suite. See
# test/e2e/README.md for what it expects to find.
e2e:
...
ci-drift:
	...
	@# A checkout driven by a local tool that runs `make check` on its behalf
	@# has a third list to keep in step, and that tool's config is not in the
...
# test run. A local gate calls this and nothing else, so its config and the CI
# job list stay one edit apart.
check: $(CHECK_LEGS)
