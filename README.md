...
- **Checksum verification**: a SHA-256 pinned on first fetch and enforced on the next, for every type with a per-version object key. An npm dist-tag pins each version it resolves to rather than the tag, so the tag stays free to move. An apt entry bodega compiles rather than downloads pins at package time, off the `.deb` the build produced. pypi and clone-mode git are the two gaps — see [docs/THREAT_MODEL.md](docs/THREAT_MODEL.md)
